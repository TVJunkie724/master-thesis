...
| Phase 4 | Runtime Credentials & Deployment State | Reduced PoC scope | Encrypted CloudConnections accept externally provisioned non-root administrator credentials; identity creation and least-privilege derivation are excluded |
...
| Phase 8 | Twin Architecture Profiles & Eventing | Complete offline | Standalone Six-layer v1, deterministic evidence, current documentation, and safe offline gates; live deployment remains separately gated |
...
| [#6](https://github.com/TVJunkie724/master-thesis/issues/6) | Scope reduced | CloudConnections remain the application credential store; the thesis PoC requires externally provisioned administrator credentials and contains no identity bootstrap workflow |
...
| Issue | Status | Notes |
|-------|--------|-------|
| [#6](https://github.com/TVJunkie724/master-thesis/issues/6) | Scope reduced | CloudConnection storage, selection, validation, binding, and private one-shot operation packages remain. The PoC does not create or rotate provider identities. See `docs/plans/2026-08-26_poc_credentials.md`. |
| [#7](https://github.com/TVJunkie724/master-thesis/issues/7) | Removed from thesis scope | Provider bootstrap adapters, sessions, plans, imports, and generated permission artifacts were removed. Provider-side setup is an explicit experiment prerequisite. |
| [#79](https://github.com/TVJunkie724/master-thesis/issues/79) | Future work | Least-privilege permission derivation and provider validation are documented as product-hardening work, not implemented by the PoC. |
...
| [#138 Implement the Flutter architecture profile workflow](https://github.com/TVJunkie724/master-thesis/issues/138) | Done offline | Strict Management-API-only catalog/selection/resolution workflow for standalone Six-layer v1, revisioned invalidation, responsive typed graph, and generic resolved review for Web and desktop. |
| [#146 Complete the Eventing functional and cost decision gate](https://github.com/TVJunkie724/master-thesis/issues/146) | Done offline | Source-backed six-bundle capability/pricing package, exact bridge, all single/pair/three-provider scenarios, S/M/L calculations, implementation blueprint, and two zero-finding reviews; live identity/capacity gates remain |
| Complete-service closure / Six-layer v1 plan | Approved immutable offline decision | PoC-focused L1-L5 bundles with Cosmos DB and Firestore retained for Azure/GCP L3 hot, provider-local L3-hot/L5, independent L4, three local plus six split online placements, raw visualization, Twin projection, finite storage jobs, workload v2, and complete-service evidence. L4-to-L5/3D, ADX/BigQuery migration, Spanner Graph, default dedicated Grafana nodes, and storage-specific CDC/outbox pipelines are not selected |
| PoC provider access / [#154](https://github.com/TVJunkie724/master-thesis/issues/154) | Reduced scope | Preconfigured administrator CloudConnections, encrypted storage, redaction, request-scoped transport, and real provider preflight. Identity provisioning and permission packs are excluded. |
| `six-layer-eventing@1` implementation | Done offline | Mandatory embedded domain events, three complete single-cloud targets, six `L3-hot == L5 != L4` targets, admissible mixed L1/L2/cool/archive paths, strict RTA/RDS v2 evidence, and deployment blocking for unresolved supervised live-capacity gates. |
| [#140 Implement six-layer-eventing@1 across the platform](https://github.com/TVJunkie724/master-thesis/issues/140) | Done offline | Standalone Six-layer profile with its own L1-L5 composition, Event Layer, source-owned bridge, provider runtime, and Management/Optimizer/Deployer/Flutter integration. |
| [#148 Produce Phase 8 evaluation evidence and final documentation](https://github.com/TVJunkie724/master-thesis/issues/148) | In revision | Standalone Six-layer evidence is digest-pinned and offline reproducible; supervised live verification remains pending. |
...
1. Preserve the validated Phase 8.10 evidence and closed-world Six-layer v1
