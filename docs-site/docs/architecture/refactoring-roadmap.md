...
| Complete-service closure / Five-layer v2 plan | Two fresh zero-finding reviews complete; user approval pending | PoC-focused L1-L5 bundles with Cosmos DB retained for Azure L3 hot, provider-local L3-hot/L5, independent L4, three local plus six split online placements, raw visualization, Twin projection, finite storage jobs, workload v2, and complete-service evidence. L4-to-L5/3D, ADX migration, Spanner Graph, default dedicated Grafana nodes, and storage-specific CDC/outbox pipelines are not selected |
