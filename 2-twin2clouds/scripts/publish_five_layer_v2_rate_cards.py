"""Publish reviewed Five-layer v2 rate cards into immutable baselines.
...
public-price observations in ``five_layer_v2_rate_card_sources.v2.json`` and
...
SOURCE_PATH = ROOT / "pricing_registry" / "five_layer_v2_rate_card_sources.v2.json"
SCHEMA_PATH = ROOT / "pricing_registry" / "five_layer_v2_rate_card.schema.json"
...
WORKLOAD_ROOT = (
    ROOT / "backend" / "contracts" / "generated" / "five-layer-workload" / "v2"
)
...
def _validate_source_manifest(source: Mapping[str, Any]) -> datetime:
    if (
        ...
        or source["schemaVersion"] != "five-layer-v2-rate-card-sources.v2"
    ):
        raise ValueError("Five-layer v2 source-manifest identity is invalid")
...
def _expected_publication() -> tuple[
    ...
    for provider in ("aws", "azure", "gcp"):
        ...
        evidence[RATE_CARD_KEY] = {
            "schema_version": "five-layer-v2-rate-card-evidence.v1",
...
def _archive_current_manifest(current: bytes) -> None:
    ...
    target = HISTORY_ROOT / f"baseline-pre-five-layer-v2-{suffix}.json"
...
def publish(*, write: bool) -> None:
    ...
    if not write:
        if BASELINE_PATH.read_bytes() != expected_manifest:
            raise SystemExit(
                "Five-layer v2 pricing baseline is stale; run with --write"
            )
        for provider, snapshot in snapshots.items():
            ...
            if not target.is_file() or target.read_bytes() != canonical_json_bytes(
                ...
                raise SystemExit(f"Five-layer v2 pricing snapshot is stale: {provider}")
        print("Five-layer v2 immutable rate cards: OK")
        return
