"""Immutable Five-layer v2 rate-card publication coverage."""
...
import scripts.publish_five_layer_v2_rate_cards as publisher
from backend.pricing_catalog_models import canonical_json_bytes
from scripts.publish_five_layer_v2_rate_cards import (
    RATE_CARD_KEY,
    ROUTE_CLASSES,
    SOURCE_PATH,
    _digest,
...
def test_predecessor_archiving_is_canonical_and_idempotent(tmp_path, monkeypatch):
    predecessor = _read(
        BASELINE_ROOT / "history" / "baseline-pre-five-layer-v2-bc44213d8101.json"
    )
