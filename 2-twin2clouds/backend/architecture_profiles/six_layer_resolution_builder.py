"""ResolvedTwinArchitecture v2 construction from complete Six-layer evidence."""
...
from .six_layer_costing import SixLayerCostEvaluation
...
class SixLayerResolutionBuilder:
    """Build a v2 resolution only when topology, capacity, and cost agree."""

    def __init__(
        self,
        *,
        expected_profile_id: str = "six-layer-eventing",
        expected_profile_version: str = "1",
        profile_label: str = "Six-layer",
    ) -> None:
        ...
        *,
        ...
        try:
            ...
            raise ArchitectureResolutionError(
                ...
                "Six-layer resolution inputs are inconsistent",
            ) from exc
    ...
    def _build(
        ...
        *,
        ...
        component_totals = [
            ...
            for component_id, amount in sorted(cost_evaluation.component_totals.items())
        ]
        edge_totals = [
        ...
        architecture = {
            ...
            "resolution_status": (
                "publishable"
                if deployment_specification["readiness"]["status"] == "deployment_ready"
            ...
            "provider_profile_refs": [
                {
                    ...
                    "digest": context.provider_profiles[provider]["content_digest"],
                    "provider": provider,
                }
            ...
            "pricing_evidence_refs": [
                dict(pricing_evidence_refs[provider]) for provider in used_providers
            ],
    ...
    def _validate_inputs(
        ...
        costs: SixLayerCostEvaluation,
        pricing_evidence_refs: Mapping[str, Mapping[str, str]],
    ) -> None:
        used_providers = {option.provider for option in candidate.candidate.components}
        ...
        if costs.monthly_total != sum(
            costs.component_totals.values(), Decimal(0)
        ) + sum(costs.edge_totals.values(), Decimal(0)):
            raise ArchitectureResolutionError(
                "ARCH_RESOLUTION_BUILD_FAILED",
                "costSummary",
                "Six-layer winner cost does not reconcile",
            )
