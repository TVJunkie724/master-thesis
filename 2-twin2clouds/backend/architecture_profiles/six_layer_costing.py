"""Exact-once atomic cost ownership and ranking inputs for Six-layer."""
...
@dataclass(frozen=True)
class SixLayerCostedCandidate:
    candidate_id: str
    canonical_assignment_key: tuple[tuple[str, str], ...]
    evaluation: SixLayerCostEvaluation
...
def _read(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"Six-layer cost registry is unavailable: {path}") from exc
    if not isinstance(value, dict):
        raise RuntimeError(f"Six-layer cost registry must be an object: {path}")
    return value
...
def _registry() -> dict[str, Any]:
    ...
    if supplied != _digest(registry):
        raise RuntimeError("Six-layer cost registry digest drifted")
...
def evaluate_six_layer_costs(
    *,
    ...
    if cost_ledger.get("schema_version") != "six-layer-cost-ledger.v1":
        raise ArchitectureResolutionError(
            "ARCH_PRICING_EVIDENCE_MISSING",
            "costLedger",
            "Six-layer cost ledger version is unsupported",
        )
    ...
    if len(supplied_component_ids) != len(set(supplied_component_ids)) or set(
        supplied_component_ids
    ...
    for quote in component_quotes:
        ...
        if (
            quote.get("cost_owner_id") != registry_component["pricing_owner_id"]
    ...
    if len(supplied_route_ids) != len(set(supplied_route_ids)) or set(
        supplied_route_ids
    ) != set(expected_routes):
        raise ArchitectureResolutionError(
            "ARCH_PRICING_EVIDENCE_MISSING",
    ...
    for quote in route_quotes:
        ...
        if (
            ...
            or tuple(quote.get("domain_flow_ids", ())) != expected.domain_flow_ids
            ...
            != {
                ...
                "destination": pricing_evidence[expected.destination_provider],
            }
            or (
                uses_event_layer
    ...
    if monthly_total != expected_total:
        raise ArchitectureResolutionError(
            ...
            "costSummary",
            "Six-layer cost aggregation does not reconcile",
        )
    return SixLayerCostEvaluation(
        currency=currency,
...
def select_lowest_cost_six_layer_candidate(
    candidates: tuple[SixLayerCostedCandidate, ...],
    ...
    if len({item.candidate_id for item in candidates}) != len(candidates):
        raise ArchitectureResolutionError(
            "ARCH_NO_ADMISSIBLE_CANDIDATE",
            "candidates",
            "Six-layer candidate IDs must be unique",
        )
    return min(
        candidates,
        key=lambda item: (
