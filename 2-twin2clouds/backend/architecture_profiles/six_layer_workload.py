"""Strict Six-layer workload and Eventing-scenario resolution."""
...
class ResolvedSixLayerWorkload:
    size: str
...
def _read(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            f"Six-layer workload contract is unavailable: {path}"
        ) from exc
    if not isinstance(value, dict):
        raise RuntimeError(f"Six-layer workload document must be an object: {path}")
    return value
...
def resolve_six_layer_workload(
    ...
    if size is None:
        raise ArchitectureResolutionError(
            "ARCH_WORKLOAD_INCOMPATIBLE",
            "workload",
            "Six-layer accepts only the immutable Small, Medium, or Large Core scenario",
        )
    ...
    return ResolvedSixLayerWorkload(
        size=size,
