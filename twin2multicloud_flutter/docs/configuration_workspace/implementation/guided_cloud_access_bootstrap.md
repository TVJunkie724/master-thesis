...
| Adapters | Deterministic no-cloud AWS, Azure, and GCP lifecycle adapters; production default is disabled. The synchronized `supervised_live` mode is represented explicitly but remains blocked by an unconfigured fail-closed adapter. Its SDK-independent orchestrator consumes the shared policy materializer, validates the exact generated v2 connection boundary, and requires secret-free rollback receipts. Management persists that validated connection before bootstrap-authority finalization, compensates persistence failure, and preserves the connection with manual-revocation state when only finalization fails. Concrete provider SDK drivers remain pending. |
