...
| Adapters | Deterministic no-cloud AWS, Azure, and GCP lifecycle adapters; production default is disabled. The synchronized `supervised_live` mode is represented explicitly but remains blocked by an unconfigured fail-closed adapter. Its future provider implementations must consume the shared SDK-free Management policy materializer, which reads only synchronized generated contracts and is also used by the repository CLI. |
