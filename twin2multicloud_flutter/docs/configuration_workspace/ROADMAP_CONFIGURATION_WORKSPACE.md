...
| 8.1 | Implemented; 8.9B final audit in progress | [Architecture profile experiment](phases/PHASE_08_1_ARCHITECTURE_PROFILE_EXPERIMENT.md), staged across Phase 8.7 UI infrastructure and the 8.9A/8.9B runtime activations | [Implementation](implementation/architecture_profile_experiment.md) now exposes Five-layer v2 and Six-layer v1 through one Management-owned workflow; the latest Flutter gate passes 888 tests, Web/macOS builds, and credential-free real-Management integration. Supervised live-capacity evidence remains deliberately open. |
