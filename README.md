...
### 2. Optional: create local credential files
...
Only create these files if you want to use the legacy local credential/seed workflow:
...
Then edit `config_credentials.json` and the GCP files with your local credentials. These files remain ignored by Git and are mounted only when you opt in via `compose.credentials.local.yaml`.
...
To opt in to the legacy local credential/seed workflow:
...
#### 4c. Sample twins (opt-in via `compose.credentials.local.yaml`)
...
To enable sample twins, start the stack with the local credentials override after creating the ignored credential files in step 2:
...
3. Create or select valid Cloud Connections for the providers you want to deploy to. Only use `compose.credentials.local.yaml` if you intentionally need the legacy root-level credential files for local testing or sample seeding.
