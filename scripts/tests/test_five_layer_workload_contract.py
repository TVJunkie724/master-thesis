"""Tests for the Five-layer Workload v2 synchronized contract."""
...
from scripts import sync_five_layer_workload_contract as contract


class FiveLayerWorkloadContractTests(unittest.TestCase):
    def setUp(self) -> None:
        self.documents = contract.expected_documents()
        self.schema = json.loads(self.documents["v2/workload.schema.json"])
        self.catalog = json.loads(
            self.documents["v2/eventing-scenario-catalog.json"]
        )
        self.small = json.loads(
            self.documents["v2/fixtures/valid/core-small.json"]
        )
    ...
    def test_three_frozen_core_scenarios_validate(self) -> None:
        for size in ("small", "medium", "large"):
            contract.validate_workload(
                json.loads(
                    self.documents[f"v2/fixtures/valid/core-{size}.json"]
                ),
                schema=self.schema,
    ...
    def test_retention_is_strictly_increasing(self) -> None:
        for hot, cool, archive in ((1, 1, 12), (2, 1, 12), (1, 12, 12)):
            ...
            self.assertEqual(raised.exception.code, "WORKLOAD_V2_RETENTION_INVALID")
    ...
    def test_eventing_scenario_is_a_closed_reference_not_inline_input(self) -> None:
        ...
        self.assertEqual(raised.exception.code, "WORKLOAD_V2_SCHEMA_INVALID")
    ...
    def test_workload_field_set_is_exact(self) -> None:
        ...
        self.assertTrue(set(contract.RETIRED_FIELDS).isdisjoint(self.schema["properties"]))
