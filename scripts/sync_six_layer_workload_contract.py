#!/usr/bin/env python3
"""Generate, validate, synchronize, and drift-check Six-layer Workload v1."""
...
SOURCE_ROOT = ROOT / "contracts" / "six-layer-workload"
...
TARGETS = (
    ...
    / "generated"
    / "six-layer-workload",
    ROOT / "twin2multicloud_flutter" / "assets" / "contracts" / "six-layer-workload",
)
SCENARIO_IDS = (
    "eventing-small-v1",
    "eventing-medium-v1",
...
EXPECTED_FILES = frozenset(
    {
        "README.md",
        "v1/workload.schema.json",
        "v1/eventing-scenario-catalog.schema.json",
        "v1/eventing-scenario-catalog.json",
        "v1/fixtures/valid/core-small.json",
        "v1/fixtures/valid/core-medium.json",
        "v1/fixtures/valid/core-large.json",
        "v1/fixtures/invalid/retired-event-flag.json",
        "v1/fixtures/invalid/inline-eventing-scenario.json",
        "v1/fixtures/invalid/missing-event-scenario.json",
        "v1/fixtures/invalid/non-increasing-retention.json",
    }
)
...
def eventing_content_digest(document: dict[str, Any]) -> str:
    ...
    spec = importlib.util.spec_from_file_location(
        "phase_08_eventing_calculator_for_six_layer_workload_v1",
        calculator_path,
    )
...
def workload_schema() -> dict[str, Any]:
    ...
    properties: dict[str, Any] = {
        "schemaVersion": {"const": "six-layer-workload.v1"},
    ...
    schema = {
        ...
        "$id": "https://twin2multicloud.local/contracts/six-layer-workload/v1/workload.schema.json",
        "title": "Six-layer Workload v1",
        "description": (
    ...
    if tuple(properties) != WORKLOAD_FIELDS:
        raise RuntimeError("Six-layer Workload v1 field set drifted")
    return schema
...
def scenario_catalog_schema() -> dict[str, Any]:
    ...
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://twin2multicloud.local/contracts/six-layer-workload/v1/eventing-scenario-catalog.schema.json",
...
def scenario_catalog() -> dict[str, Any]:
    ...
    by_id = {str(item.get("scenario_id")): copy.deepcopy(item) for item in scenarios}
...
def valid_workloads() -> dict[str, dict[str, Any]]:
    ...
    for scenario in scenarios:
        ...
        workloads[size] = {
            ...
            "deviceSendingIntervalInMinutes": scenario["telemetry_interval_seconds"]
            / 60,
...
def validate_workload(
    ...
    *,
    ...
    known_scenarios = {item["scenario_id"] for item in catalog["scenarios"]}
    if workload["eventingScenarioId"] not in known_scenarios:
        raise WorkloadContractError(
            "SIX_LAYER_WORKLOAD_V1_EVENTING_SCENARIO_UNKNOWN",
...
def expected_documents() -> dict[str, bytes]:
    ...
    documents: dict[str, object] = {
        "v1/workload.schema.json": schema,
        "v1/eventing-scenario-catalog.schema.json": catalog_schema,
        "v1/eventing-scenario-catalog.json": catalog,
        **{
            f"v1/fixtures/valid/core-{size}.json": workload
            for size, workload in workloads.items()
        },
        "v1/fixtures/invalid/retired-event-flag.json": invalid_retired,
        "v1/fixtures/invalid/inline-eventing-scenario.json": invalid_inline,
        "v1/fixtures/invalid/missing-event-scenario.json": invalid_missing,
        "v1/fixtures/invalid/non-increasing-retention.json": invalid_retention,
    }
    rendered = {
    ...
    rendered["README.md"] = (
        ...
        "Generated by `scripts/sync_six_layer_workload_contract.py`; do not edit generated copies.\n"
    ).encode()
    return rendered
...
def validate_expected(documents: dict[str, bytes]) -> None:
    if set(documents) != EXPECTED_FILES:
        raise RuntimeError("Six-layer workload generated file set drifted")
    schema = json.loads(documents["v1/workload.schema.json"])
    catalog_schema = json.loads(documents["v1/eventing-scenario-catalog.schema.json"])
    catalog = json.loads(documents["v1/eventing-scenario-catalog.json"])
    ...
    if catalog_errors:
        raise RuntimeError(
            f"Eventing scenario catalog is invalid: {catalog_errors[0].message}"
        )
    ...
    for path in sorted(path for path in documents if "/invalid/" in path):
        try:
            validate_workload(
                json.loads(documents[path]), schema=schema, catalog=catalog
            )
...
def _check_tree(root: Path, documents: dict[str, bytes], marker: str | None) -> None:
    ...
    if actual != documents:
        raise RuntimeError(f"Six-layer workload contract drift: {root}")
    if marker is not None:
        marker_path = root / ".contract-sha256"
        if not marker_path.exists() or marker_path.read_text().strip() != marker:
            raise RuntimeError(f"Six-layer workload marker drift: {root}")
...
def main() -> int:
    ...
    print(
        f"six-layer-workload: OK (source_digest={marker}, generated_copies={len(TARGETS)})"
    )
    return 0
