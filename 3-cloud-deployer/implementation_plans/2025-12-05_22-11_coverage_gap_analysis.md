[Content of implementation_plan.md]
    - `test_create_hot_dynamodb_table`: Verify table creation and primary key schema.
- **S3**:
    - `test_create_cold_s3_bucket`: Verify## Proposed Changes
...
#### [MODIFY] [test_rest_api.py](file:///d:/Git/master-thesis/3-cloud-deployer/tests/api/test_rest_api.py)
   - Add `test_recreate_updated_events`: Verify the API endpoint calls the underlying deployers correctly.overage.
    - `test_create_cold_s3_bucket`: Verify bucket creation.
    - `test_create_archive_s3_bucket`: Verify bucket creation.
- **Movers**:
    - `test_create_hot_cold_mover_lambda_function`: Verify lambda creation.
