"""Frozen GCP Six-layer Grafana image and dashboard contract tests."""
...
def test_datasource_is_read_only_bounded_and_secret_backed():
    ...
    assert (
        "customHealthCheckUrl: ${RAW_HISTORY_READER_URL}/raw-history-health/v1"
        in datasource
    )
...
def test_dashboard_has_exactly_the_two_reviewed_history_panels():
    ...
    for panel, bucket, time_column in zip(
        ...
        assert target["url"].startswith("__RAW_HISTORY_READER_URL__/raw-history/v1?")
...
def test_entrypoint_fails_closed_until_content_and_reader_probes_pass():
    ...
    assert (
        "probe_reader 3600 '2026-01-01T00:00:00Z' '2026-01-31T00:00:00Z'" in entrypoint
    )
