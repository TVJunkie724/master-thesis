"""Azure Six-layer canonical envelope parity tests."""
...
SPEC = importlib.util.spec_from_file_location("azure_six_layer_core", CORE_PATH)
...
FUNCTION_SPEC = importlib.util.spec_from_file_location(
    "azure_six_layer_function_app", FUNCTION_APP_PATH
)
...
def test_local_azure_processing_enqueues_processed_then_persists_projects_and_rules(
    ...
    monkeypatch.setenv("SIX_LAYER_L2_PROVIDER", "azure")
    monkeypatch.setenv("SIX_LAYER_TWIN_PROVIDER", "azure")
    monkeypatch.setattr(
        function_app,
        "_invoke_processor_extension",
        lambda _event: {"value": 4, "quality": "accepted"},
    )
    monkeypatch.setattr(function_app, "_publish_eventing_stream", enqueue)
...
def test_rule_matches_have_distinct_ids_and_drive_all_embedded_paths(monkeypatch):
    ...
    enqueued = []
    monkeypatch.setenv("SIX_LAYER_L1_PROVIDER", "azure")
...
def test_remote_processing_projection_and_outcomes_use_typed_outboxes(monkeypatch):
    ...
    telemetry = []
    control = []
    monkeypatch.setenv("SIX_LAYER_HOT_PROVIDER", "aws")
    monkeypatch.setenv("SIX_LAYER_TWIN_PROVIDER", "aws")
    monkeypatch.setattr(
        function_app,
        "_invoke_processor_extension",
        lambda _event: {"value": 4, "quality": "accepted"},
    ...
    assert [event["event_type"] for event in telemetry] == ["telemetry.processed.v1"]

    processed = core.build_processed_event(source, {"value": 4, "quality": "accepted"})
    monkeypatch.setenv("SIX_LAYER_HOT_PROVIDER", "azure")
...
def test_terminal_outcome_storage_is_idempotent_and_non_rollup(monkeypatch):
    ...
    container = MagicMock()
    monkeypatch.setenv("SIX_LAYER_HOT_PROVIDER", "azure")
    monkeypatch.setenv("SIX_LAYER_HOT_BOUNDARY_DAYS", "30")
    monkeypatch.setenv("SIX_LAYER_STORAGE_TASK_COUNT", "4")
...
def test_envelope_field_set_matches_aws_six_layer_runtime():
    aws_path = (
        Path(__file__).resolve().parents[3]
        / "src"
        / "providers"
    ...
    aws_spec = importlib.util.spec_from_file_location("aws_six_layer", aws_path)
...
def test_adt_projection_creates_a_missing_poc_twin(monkeypatch):
    ...
    monkeypatch.setattr(function_app, "_ADT_CLIENT", client)
    monkeypatch.setenv("SIX_LAYER_HOT_PROVIDER", "gcp")
