...
class TestLoadConfig:
    ...
    def test_minimal_config(self, tmp_path):
        ...
        assert (
            "layer_3_hot_to_cold_interval_days" not in result
            or result.get("layer_3_hot_to_cold_interval_days") is None
        )
...
class TestLoadCredentials:
    ...
    def test_requires_credentials_file(self, tmp_path):
        ...
        with pytest.raises(
            ConfigurationError, match="config_credentials.json not found"
        ):
    ...
    def test_requires_azure_fields(self, tmp_path):
        ...
        with pytest.raises(
            ConfigurationError, match="Missing required Azure credential"
        ):
            _load_credentials(tmp_path)
...
class TestLoadProviders:
    ...
    def test_requires_all_layer_providers(self, tmp_path):
        ...
        with pytest.raises(
            ConfigurationError, match="Missing required provider config"
        ):
            _load_providers(tmp_path)
...
class TestLoadIotDevices:
    ...
    def test_requires_devices_file(self, tmp_path):
        ...
        with pytest.raises(
            ConfigurationError, match="config_iot_devices.json not found"
        ):
    ...
    def test_loads_devices_array(self, tmp_path):
        ...
        devices = [{"id": "sensor-1", "properties": [{"name": "temp"}]}]
        devices_file = tmp_path / "config_iot_devices.json"
...
def test_load_platform_user_maps_existing_azure_principal_without_credentials(tmp_path):
    ...
    assert result["azure_layer_access_principal_object_id"] == (
...
class TestGenerateTfvars:
    ...
    def complete_project(self, tmp_path):
        ...
        (tmp_path / "config_iot_devices.json").write_text(
            json.dumps([{"id": "sensor-1", "properties": []}])
        )
