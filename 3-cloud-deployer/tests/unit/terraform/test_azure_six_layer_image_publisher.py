"""Offline tests for automatic Azure Six-layer image publication."""
...
class _Transport:
    ...
    def request(self, method, url, **kwargs):
        ...
        if "taskRuns/" in url and method == "PUT":
            self.image_name = kwargs["json"]["properties"]["runRequest"]["imageNames"][
                0
            ]
            self.created = True
...
def _publisher(tmp_path, transport):
    return AzureSixLayerImagePublisher(
...
def test_publisher_uploads_context_and_returns_task_run_digest(tmp_path):
    ...
    images = publisher.publish((AzureSixLayerImageRequest("storage-mover", context),))
    ...
    assert (
        publisher.publish((AzureSixLayerImageRequest("storage-mover", context),))
        == images
    )
    assert (
        len([call for call in transport.calls if call[1].startswith("https://upload")])
        == 1
    )
...
def test_selection_and_tfvars_are_bounded_to_the_azure_mover():
    assert azure_six_layer_container_deployment(_tfvars()) is True
    assert (
        azure_six_layer_container_deployment(
    ...
    assert image_tfvars({"storage-mover": image}, _tfvars()) == {
        "azure_six_layer_storage_mover_image": image
    }
