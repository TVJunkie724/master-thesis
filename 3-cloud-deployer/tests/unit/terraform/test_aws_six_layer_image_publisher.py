"""Offline tests for automatic AWS Six-layer image publication."""
...
def test_publisher_uploads_conditional_context_and_returns_digest(tmp_path):
    ...
    builds = _CodeBuild()
    ecr = _Ecr()
    repository = "123456789012.dkr.ecr.eu-central-1.amazonaws.com/factory-six-layer"
    publisher = AwsSixLayerImagePublisher(
        project_path=tmp_path,
        region="eu-central-1",
        source_bucket="build-bucket",
        repository_url=repository,
        repository_name="factory-six-layer",
        codebuild_project="factory-six-layer-images",
        s3_client=s3,
    ...
    images = publisher.publish((AwsSixLayerImageRequest("storage-mover", context),))
    ...
    assert ecr.requests[-1]["repositoryName"] == "factory-six-layer"
    assert publisher.evidence_path.stat().st_mode & 0o077 == 0

    assert (
        publisher.publish((AwsSixLayerImageRequest("storage-mover", context),))
        == images
    )
...
def test_selection_and_tfvars_are_bounded_to_graph_selected_images(tmp_path):
    assert aws_six_layer_container_deployment(_tfvars()) is True
    assert (
        aws_six_layer_container_deployment(
            {**_tfvars(), "architecture_profile_version": "2"}
        )
        is False
    )
    image = (
        "123456789012.dkr.ecr.eu-central-1.amazonaws.com/factory-six-layer@sha256:"
        + "b" * 64
    )
    assert image_tfvars({"storage-mover": image}, _tfvars()) == {
        "aws_six_layer_storage_mover_image": image
    }
    ...
    bridge_context = tmp_path / ".build" / "aws" / "six-layer-domain-bridge.zip"
    bridge_context.parent.mkdir(parents=True)
    bridge_context.write_bytes(b"bridge")

    assert aws_six_layer_bridge_deployment(bridge_only) is True
    assert image_requests(tmp_path, bridge_only) == (
        AwsSixLayerImageRequest("bridge", bridge_context),
    )
    assert image_tfvars({"bridge": image}, bridge_only) == {
        "aws_six_layer_bridge_image": image
    }
    ...
    assert aws_six_layer_bridge_deployment(inbound_only) is False
    assert aws_six_layer_container_deployment(inbound_only) is False
...
def test_storage_and_bridge_images_are_both_required_when_selected(tmp_path):
    ...
    storage_context = build_root / "six-layer-domain-storage-mover.zip"
    bridge_context = build_root / "six-layer-domain-bridge.zip"
    storage_context.write_bytes(b"storage")
    bridge_context.write_bytes(b"bridge")

    assert image_requests(tmp_path, values) == (
        AwsSixLayerImageRequest("storage-mover", storage_context),
        AwsSixLayerImageRequest("bridge", bridge_context),
    )
    image = (
        "123456789012.dkr.ecr.eu-central-1.amazonaws.com/factory-six-layer@sha256:"
    ...
    assert image_tfvars({"storage-mover": image, "bridge": image}, values) == {
        "aws_six_layer_storage_mover_image": image,
        "aws_six_layer_bridge_image": image,
    }
