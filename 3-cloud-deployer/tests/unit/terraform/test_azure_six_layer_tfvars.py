"""Six-layer Azure package-to-Terraform input tests."""
...
def test_loads_content_verified_azure_six_layer_graph_package(monkeypatch, tmp_path):
    package = tmp_path / ".build" / "azure" / "six-layer-domain.zip"
    package.parent.mkdir(parents=True)
    package.write_bytes(b"deterministic-azure-six-layer-package")
        ...
        json.dumps(
            {
                ...
                "built_packages": [
                    {
                        "package_id": "azure_six-layer-domain",
    ...
    monkeypatch.setattr(
        ...
        lambda _graph: (
            {"aws": (), "azure": ("six-layer-domain",), "gcp": ()},
    ...
    assert result["azure_six_layer_zip_path"] == str(package)
