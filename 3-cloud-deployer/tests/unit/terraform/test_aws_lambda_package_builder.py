    ...
    def test_handler_function_exists(self, lambda_func_dir, shared_dir, tmp_path):
        ...
        with zipfile.ZipFile(output) as zf:
            ...
            assert "lambda_handler" in func_names, (
                "lambda_function.py must define lambda_handler()"
            )
...
class TestRealLambdaFunctions:
    ...
    def test_all_lambdas_have_handler_file(self, lambda_functions_dir):
        ...
        for func_dir in lambda_functions_dir.iterdir():
            ...
            assert handler_file.exists(), (
                f"Missing lambda_function.py in {func_dir.name}"
            )
    ...
    def test_all_lambdas_define_lambda_handler(self, lambda_functions_dir):
        ...
        for func_dir in lambda_functions_dir.iterdir():
            ...
            assert "lambda_handler" in func_names, (
                f"{func_dir.name} must define lambda_handler()"
            )
    ...
    def test_dispatcher_zip_structure(self, lambda_functions_dir, shared_dir, tmp_path):
        ...
        with zipfile.ZipFile(output) as zf:
            ...
            assert any("env_utils" in n for n in names), (
                "Missing env_utils shared module"
            )
    ...
    def test_five_layer_v2_multi_handler_package(self, tmp_path):
        ...
        with zipfile.ZipFile(package) as archive:
            ...
            assert "phase8_eventing/aws/runtime.py" in archive.namelist()
            assert "phase8_eventing/azure/bridge.py" in archive.namelist()
            assert "phase8_eventing/azure/runtime.py" in archive.namelist()
            assert "phase8_eventing/bridge_application.py" in archive.namelist()
            assert "phase8_eventing/destination_identity.py" in archive.namelist()
            assert "phase8_eventing/destination_publishers.py" in archive.namelist()
            assert "phase8_eventing/gcp/bridge.py" in archive.namelist()
            assert "phase8_eventing/gcp/runtime.py" in archive.namelist()
