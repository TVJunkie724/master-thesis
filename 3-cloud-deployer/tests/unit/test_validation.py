...
class TestValidation(unittest.TestCase):
    ...
    def test_validate_config_credentials_valid(self):
        ...
        content = {
            ...
            "azure": {
                ...
                "azure_preparation_client_id": "preparation-x",
                "azure_preparation_client_secret": "preparation-secret",
                "azure_region": "x",
                "azure_region_iothub": "x",
    ...
    def test_validate_config_credentials_missing_field(self):
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_config_content(
                CONSTANTS.CONFIG_CREDENTIALS_FILE, content
            )
        self.assertIn(
            "Missing credential field 'aws_secret_access_key'", str(cm.exception)
        )
    ...
    def test_validate_config_file_list_malformed(self):
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_config_content(
                CONSTANTS.CONFIG_IOT_DEVICES_FILE, [{"other": "1"}]
        ...
        content = [
            {
                ...
                "action": {
                    "type": "lambda",
                    "functionName": "x",
                    "feedback": {"iotDeviceId": "d", "payload": "p"},
                },
            }
        ]
        validator.validate_config_content(CONSTANTS.CONFIG_EVENTS_FILE, content)
    ...
    def test_validate_config_hierarchy(self):
        ...
        content = [
            {"id": "1", "type": "component", "name": "c1", "componentTypeId": "type1"}
        ]
        validator.validate_aws_hierarchy_content(content)
    ...
    def test_validate_state_machine_invalid_json(self):
        with self.assertRaises(ValueError):
            validator.validate_state_machine_content(
                CONSTANTS.AWS_STATE_MACHINE_FILE, "{"
            )
    ...
    def test_validate_state_machine_content_azure_special(self):
        ...
        validator.validate_state_machine_content(
            CONSTANTS.AZURE_STATE_MACHINE_FILE, valid_azure
        )

        # 2. Invalid - missing definition wrapper entirely
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                CONSTANTS.AZURE_STATE_MACHINE_FILE, {"other": {}}
            )
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                CONSTANTS.AZURE_STATE_MACHINE_FILE,
    ...
    def test_validate_state_machine_content_google(self):
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                CONSTANTS.GOOGLE_STATE_MACHINE_FILE, {"steps": []}
            )
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                CONSTANTS.GOOGLE_STATE_MACHINE_FILE, {"main": {}}
            )
        ...
        validator.validate_state_machine_content(
            CONSTANTS.GOOGLE_STATE_MACHINE_FILE, valid_gcp
        )
        ...
        validator.validate_state_machine_content(
            CONSTANTS.GOOGLE_STATE_MACHINE_FILE, valid_gcp_list
        )
        ...
        with self.assertRaises(ValueError) as cm:
            ...
            validator.validate_state_machine_content(
                CONSTANTS.GOOGLE_STATE_MACHINE_FILE, duplicate_steps
            )
        ...
        with self.assertRaises(ValueError) as cm:
            ...
            validator.validate_state_machine_content(
                CONSTANTS.GOOGLE_STATE_MACHINE_FILE, duplicate_list
            )
        self.assertIn("Duplicate step names", str(cm.exception))
    ...
    def test_validate_state_machine_content_aws(self):
        ...
        validator.validate_state_machine_content(
            CONSTANTS.AWS_STATE_MACHINE_FILE, valid_aws
        )
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                CONSTANTS.AWS_STATE_MACHINE_FILE, {"StartAt": "Init"}
            )
        self.assertIn("Missing required key", str(cm.exception))
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                ...
                {
                    "StartAt": "NonExistent",
                    "States": {"ActualState": {"Type": "Succeed"}},
                },
            )
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                CONSTANTS.AWS_STATE_MACHINE_FILE,
                {"StartAt": "Init", "States": ["not", "an", "object"]},
            )
    ...
    def test_state_machine_gcp_missing_main_and_steps_edge_case(self):
        ...
        with self.assertRaises(ValueError) as cm:
            validator.validate_state_machine_content(
                CONSTANTS.GOOGLE_STATE_MACHINE_FILE, {}
            )
        error_msg = str(cm.exception)
    ...
    def test_validate_python_code_azure_invalid(self):
        ...
        self.assertIn(
            "main(req)", str(cm.exception)
        )  # Updated: new message uses "signature: main(req)"
    ...
    def _create_zip_with_configs(self, configs=None, extra_files=None):
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zf:
            ...
            providers_config = (
                configs.get(CONSTANTS.CONFIG_PROVIDERS_FILE) if configs else None
            )
            ...
            if not already_in_extras:
                if layer_4_provider == "aws":
                    ...
                    zf.writestr(
                        azure_hierarchy_path,
                        json.dumps({"models": [], "twins": [], "relationships": []}),
                    )
            ...
            iot_config = (
                configs.get(CONSTANTS.CONFIG_IOT_DEVICES_FILE) if configs else None
            )
            if iot_config and func_dir:
                ...
                proc_file = processor_file_map.get(
                    layer_2_provider, "lambda_function.py"
                )
                processor_code = processor_code_map.get(
                    layer_2_provider, processor_code_map["aws"]
                )
            ...
            if func_dir:
                ...
                # Only add if not already in extra_files
                if not extra_files or not any(
                    f.startswith(func_dir + "/") for f in extra_files
                ):
    ...
    def test_validate_zip_feedback_success(self):
        ...
        zip_buf = self._create_zip_with_configs(
            {CONSTANTS.CONFIG_OPTIMIZATION_FILE: opt}, extras
        )
        validator.validate_project_zip(zip_buf)
    ...
    def test_validate_zip_event_checks_missing_code(self):
        ...
        events = [
            {"condition": "x", "action": {"type": "lambda", "functionName": "my-func"}}
        ]
        configs = {
            CONSTANTS.CONFIG_OPTIMIZATION_FILE: opt,
            CONSTANTS.CONFIG_EVENTS_FILE: events,
        }
    ...
    def test_validate_project_directory_workflow_checks(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (project_dir / "config_providers.json").write_text(
                json.dumps(
                    {
                        "layer_1_provider": "aws",
                        "layer_2_provider": "aws",
            ...
            (project_dir / "config_optimization.json").write_text(
                json.dumps(
                    {
                        "result": {
                            "inputParamsUsed": {"triggerNotificationWorkflow": True}
                        }
                    }
                )
            )
            ...
            (project_dir / "lambda_functions/placeholder.txt").write_text(
                "dummy"
            )  # Needs a file to pass check_provider_function_directory
    ...
    def test_get_provider_unknown_func_raises(self, mock_json, mock_open, mock_exists):
        ...
        with self.assertRaises(ValueError) as cm:
            validator.get_provider_for_function(
                "p", "unknown-func", project_path="/app"
            )
    ...
    def test_validate_zip_credentials_missing_for_provider(self):
        ...
        extras = {
            "twin_hierarchy/azure_hierarchy.json": json.dumps(
    ...
    def test_validate_zip_credentials_all_providers_present(self):
        ...
        credentials = {
            ...
            "azure": {
                ...
                "azure_preparation_client_id": "preparation-c",
                "azure_preparation_client_secret": "preparation-s",
                "azure_region": "eastus",
    ...
    def test_validate_zip_hierarchy_google_no_file_required(self):
        ...
        zip_buffer.seek(0)
        validator.validate_project_zip(
            zip_buffer
        )  # Should pass for Google (no L4 hierarchy)
...
class TestProviderDirectoryValidation(unittest.TestCase):
    ...
    def test_provider_directory_exists(self):
        ...
        from src.validation.core import (
            check_provider_function_directory,
            ValidationContext,
        )
        import constants as CONSTANTS
...
class TestStateMachineValidation(unittest.TestCase):
    ...
    def test_state_machine_skipped_when_workflow_disabled(self):
        ...
        ctx = ValidationContext()
        ctx.opt_config = {
            "result": {"inputParamsUsed": {"triggerNotificationWorkflow": False}}
        }
    ...
    def test_state_machine_aws_validates_correct_file(self):
        ...
        mock_accessor.read_text.return_value = json.dumps(
            {"StartAt": "Init", "States": {"Init": {"Type": "Succeed"}}}
        )
    ...
    def test_state_machine_google_validates_correct_file(self):
        ...
        mock_accessor.read_text.return_value = (
            "main:\n  - init:\n      assign:\n        - result: ok\n"
        )
