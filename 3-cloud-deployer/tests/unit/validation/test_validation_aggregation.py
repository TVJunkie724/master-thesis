...
class TestAggregationHappyPaths:
    ...
    def test_phase8_project_uses_profile_owned_runtime_artifacts(self):
        result = run_all_checks_aggregated(
            _phase8_aws_project(),
            ValidationContext(architecture_profile=("six-layer-eventing", "1")),
        )
...
class TestDeploymentManifestValidation:
    ...
    def test_manifest_exposes_phase8_profile_to_directory_validation(
        ...
        assert ctx.architecture_profile == ("six-layer-eventing", "1")
    ...
    def test_manifest_rejects_credential_payload_keys(self):
        manifest = self._valid_manifest()
        manifest["credentials"]["sources"]["aws_secret_access_key"] = (
            "do-not-report-value"
        )
        accessor = MockAccessor(self._base_project_files(manifest))
        ctx = self._context(manifest)
...
class TestBinaryRead:
    ...
    def test_read_binary_returns_bytes(self):
        ...
        accessor = MockAccessor({"scene_assets/scene.glb": binary_content})
    ...
    def test_read_text_and_binary_same_file(self):
        ...
        accessor = MockAccessor({"config.json": text_content})
