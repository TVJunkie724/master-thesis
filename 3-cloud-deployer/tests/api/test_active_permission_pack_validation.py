...
def test_aws_v2_checker_uses_every_frozen_pack_action_once():
    ...
    expected = {
        action for group in pack["policy_inputs"] for action in group["actions"]
    }
    required = _get_all_required_permissions(contract)
    actual = {action for service in required.values() for action in service["actions"]}
...
def test_azure_v2_checker_uses_exact_management_and_data_actions():
    ...
    assert (
        contract["thesis_demo_v2"]["required_actions"] == pack["role_inputs"]["actions"]
    )
    assert (
        contract["thesis_demo_v2"]["required_data_actions"]
        == pack["role_inputs"]["data_actions"]
    )
...
def test_gcp_v2_checks_every_fixed_api_without_listing_or_mutation(
    mock_service_usage_client,
):
    ...
    assert result["baseline_id"] == "gcp.phase8-api-baseline.v1"
    assert result["baseline_owner"] == "bootstrap.gcp.admin-v3"
    assert result["retain_enabled"] is True
    assert result["required_permission"] == "serviceusage.services.get"
    assert result["by_layer"]["phase8_api_baseline"] == {
        "status": "valid",
        "present_apis": baseline["services"],
        "missing_apis": [],
    }
    assert client.get_service.call_args_list == [
        call(name=f"projects/test-project/services/{service}")
