...
def update_function(local_function_name, environment=None):
  if local_function_name == "default-processor":
    compiled_function = util.compile_lambda_function(os.path.join(globals_aws.lambda_functions_path, local_function_name))
    ...
    for iot_device in globals.config_iot_devices:
      ...
      waiter = globals_aws.aws_lambda_client.get_waiter("function_updated")
      waiter.wait(FunctionName=function_name)

      if environment is not None:
        globals_aws.aws_lambda_client.update_function_configuration(
          FunctionName=function_name,
  ...
  globals_aws.aws_lambda_client.update_function_code(
    FunctionName=function_name,
    ZipFile=util.compile_lambda_function(os.path.join(globals_aws.lambda_functions_path, local_function_name)),
    Publish=True
  )
...
def fetch_logs(local_function_name, n=10, filter_system_logs=True):
  ...
  streams = globals_aws.aws_logs_client.describe_log_streams( logGroupName=log_group, orderBy="LastEventTime", descending=True, limit=1)
  latest_stream = streams["logStreams"][0]["logStreamName"]

  events = globals_aws.aws_logs_client.get_log_events(logGroupName=log_group, logStreamName=latest_stream, limit=n, startFromHead=False)
...
def invoke_function(local_function_name, payload={}, sync=True):
  ...
  response = globals_aws.aws_lambda_client.invoke(
      FunctionName=function_name,
