...
def create_iot_thing(iot_device):
  ...
  dir = f"{globals_aws.iot_data_path}/{iot_device['id']}/"
...
def destroy_iot_thing(iot_device):
  ...
  try:
    shutil.rmtree(f"{globals_aws.iot_data_path}/{iot_device['id']}")
...
def create_processor_lambda_function(iot_device):
  ...
  if os.path.exists(os.path.join(globals.project_path(), globals_aws.lambda_functions_path, function_name)):
  ...
  globals_aws.aws_lambda_client.create_function(
    ...
    Code={"ZipFile": util.compile_lambda_function(os.path.join(globals_aws.lambda_functions_path, function_name_local))},
    Description="",
    Timeout=3, # seconds
    MemorySize=128, # MB
    Publish=True,
