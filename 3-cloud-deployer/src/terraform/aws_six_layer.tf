# AWS implementation of six-layer-eventing@1.
#
...
locals {
  aws_six_layer_l1_enabled      = local.six_layer_enabled && var.layer_1_provider == "aws"
  aws_six_layer_l2_enabled      = local.six_layer_enabled && var.layer_2_provider == "aws"
  aws_six_layer_hot_enabled     = local.six_layer_enabled && var.layer_3_hot_provider == "aws"
  aws_six_layer_cool_enabled    = local.six_layer_enabled && var.layer_3_cold_provider == "aws"
  aws_six_layer_archive_enabled = local.six_layer_enabled && var.layer_3_archive_provider == "aws"
  aws_six_layer_l4_enabled      = local.six_layer_enabled && var.layer_4_provider == "aws"
  aws_six_layer_l5_enabled      = local.six_layer_enabled && var.layer_5_provider == "aws"

  aws_six_layer_event_enabled = (
    local.aws_six_layer_l1_enabled || local.aws_six_layer_l2_enabled ||
    local.aws_six_layer_hot_enabled || local.aws_six_layer_l4_enabled ||
  ...
  aws_six_layer_embedded_event_enabled = (
  ...
  aws_six_layer_storage_mover_enabled = (
    local.aws_six_layer_hot_enabled ||
    (local.aws_six_layer_cool_enabled && var.layer_3_archive_provider != "aws")
  )
  aws_six_layer_storage_task_count = tonumber(lookup(
    var.resolved_component_dimensions,
    "dimension.aws.aws.ecs-fargate-storage-mover.task_count",
    "0",
  ))
  aws_six_layer_storage_jobs = merge(
    local.aws_six_layer_hot_enabled ? {
      hot-to-cool = {
        source_provider      = "aws"
  ...
  aws_six_layer_storage_schedule_tasks = {
    for item in flatten([
      for transition, job in local.aws_six_layer_storage_jobs : [
        for task_index in range(local.aws_six_layer_storage_task_count) : {
  ...
  aws_six_layer_object_store_enabled = local.aws_six_layer_cool_enabled || local.aws_six_layer_archive_enabled

  aws_six_layer_outbound_event_routes = {
    for route in var.resolved_cross_cloud_routes : route.route_id => route
    if local.six_layer_enabled && route.execution_kind == "source_event_forwarder" && route.source_provider == "aws"
  }
  aws_six_layer_image_foundation_enabled = (
    local.aws_six_layer_storage_mover_enabled || length(local.aws_six_layer_outbound_event_routes) > 0
  )
  aws_six_layer_inbound_event_routes = {
    for route in var.resolved_cross_cloud_routes : route.route_id => route
    if local.six_layer_enabled && route.execution_kind == "source_event_forwarder" && route.destination_provider == "aws"
  }
  aws_six_layer_remote_telemetry_outbound = anytrue([
    for route in values(local.aws_six_layer_outbound_event_routes) :
    route.channel_class == "telemetry" && !startswith(route.logical_edge_id, "edge.eventing-to-")
  ])
  aws_six_layer_event_remote_telemetry_outbound = anytrue([
    for route in values(local.aws_six_layer_outbound_event_routes) :
    route.channel_class == "telemetry" && startswith(route.logical_edge_id, "edge.eventing-to-")
  ])
  aws_six_layer_remote_telemetry_inbound = anytrue([
    for route in values(local.aws_six_layer_inbound_event_routes) :
    route.channel_class == "telemetry" && !endswith(route.logical_edge_id, "-to-eventing")
  ])
  aws_six_layer_remote_telemetry_routes = {
    for direction, enabled in {
      inbound  = local.aws_six_layer_remote_telemetry_inbound
      outbound = local.aws_six_layer_remote_telemetry_outbound
    } : direction => direction if enabled
  }
  aws_six_layer_remote_telemetry_enabled = length(local.aws_six_layer_remote_telemetry_routes) > 0

  aws_six_layer_remote_control_outbound = anytrue([
    for route in values(local.aws_six_layer_outbound_event_routes) :
    route.channel_class == "control" && !startswith(route.logical_edge_id, "edge.eventing-to-")
  ])
  aws_six_layer_event_remote_control_outbound = anytrue([
    for route in values(local.aws_six_layer_outbound_event_routes) :
    route.channel_class == "control" && startswith(route.logical_edge_id, "edge.eventing-to-")
  ])
  aws_six_layer_remote_control_inbound = anytrue([
    for route in values(local.aws_six_layer_inbound_event_routes) :
    route.channel_class == "control" && !endswith(route.logical_edge_id, "-to-eventing")
  ])
  aws_six_layer_remote_control_routes = {
    for direction, enabled in {
      inbound  = local.aws_six_layer_remote_control_inbound
      outbound = local.aws_six_layer_remote_control_outbound
    } : direction => direction if enabled
  }
  aws_six_layer_remote_control_enabled = length(local.aws_six_layer_remote_control_routes) > 0
  aws_six_layer_domain_consumer_enabled = (
    local.aws_six_layer_remote_telemetry_inbound ||
    local.aws_six_layer_remote_control_inbound ||
    (local.aws_event_enabled && local.aws_six_layer_hot_enabled)
  )
  ...
  aws_six_layer_tags = merge(local.aws_common_tags, {
    ArchitectureProfile = "${var.architecture_profile_id}@${var.architecture_profile_version}"
  })
}

resource "terraform_data" "aws_six_layer_processor_extension_guard" {
  count = local.aws_six_layer_l2_enabled ? 1 : 0

  input = {
    package_count = length(local.aws_six_layer_processor_extensions)
  }

  lifecycle {
    precondition {
      condition     = length(local.aws_six_layer_processor_extensions) == 1
      error_message = "AWS Six-layer requires exactly one validated processor.telemetry@1 package."
    }
  }
}
...
resource "aws_cloudwatch_log_group" "aws_aws_cloudwatch" {
  count             = local.aws_six_layer_event_enabled || local.aws_six_layer_storage_mover_enabled || local.aws_six_layer_l5_enabled ? 1 : 0
  name              = "/twin2multicloud/${local.aws_six_layer_name}/six-layer"
...
resource "aws_sqs_queue" "aws_aws_sqs_fifo" {
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.aws_six_layer_embedded_failure[0].arn
    maxReceiveCount     = 5
  })
  tags = local.aws_six_layer_tags
}

resource "aws_sqs_queue" "aws_six_layer_embedded_failure" {
  count                      = local.aws_six_layer_embedded_event_enabled || local.aws_six_layer_remote_control_inbound ? 1 : 0
  name                       = "${local.aws_six_layer_name}-embedded-failure.fifo"
  fifo_queue                 = true
  message_retention_seconds  = 1209600
  receive_wait_time_seconds  = 20
  visibility_timeout_seconds = 60
...
resource "aws_sqs_queue" "aws_six_layer_remote_failure" {
  count                      = local.aws_six_layer_remote_telemetry_inbound || local.aws_six_layer_remote_control_inbound ? 1 : 0
  name                       = "${local.aws_six_layer_name}-remote-failure"
  message_retention_seconds  = 1209600
  receive_wait_time_seconds  = 20
  visibility_timeout_seconds = 60
  tags                       = local.aws_six_layer_tags
...
resource "aws_sqs_queue" "aws_six_layer_remote_control" {
  count                       = local.aws_six_layer_remote_control_inbound ? 1 : 0
  name                        = "${local.aws_six_layer_name}-remote-control-landing.fifo"
  fifo_queue                  = true
  content_based_deduplication = false
  message_retention_seconds   = 1209600
  receive_wait_time_seconds   = 20
...
resource "aws_sqs_queue_policy" "aws_six_layer_remote_control" {
  count     = local.aws_six_layer_remote_control_inbound ? 1 : 0
  queue_url = aws_sqs_queue.aws_six_layer_remote_control[0].url
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      ...
      Resource  = aws_sqs_queue.aws_six_layer_remote_control[0].arn
      Condition = {
        ArnEquals = {
...
resource "aws_kinesis_stream" "aws_aws_kinesis_only_for_reviewed_remote_telemetry_edge" {
  for_each         = local.aws_six_layer_remote_telemetry_routes
  name             = "${local.aws_six_layer_name}-remote-telemetry-${each.key}"
  shard_count      = 1
  retention_period = 24
...
resource "aws_sns_topic" "aws_aws_sns_fifo_only_for_reviewed_remote_control_edge" {
  for_each                    = local.aws_six_layer_remote_control_routes
  name                        = "${local.aws_six_layer_name}-remote-control-${each.key}.fifo"
  fifo_topic                  = true
  content_based_deduplication = false
  tags                        = local.aws_six_layer_tags
}

resource "aws_iam_role" "aws_six_layer_lambda" {
  count = local.aws_six_layer_event_enabled || local.aws_six_layer_l5_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-lambda-${local.deployment_suffix}"
...
resource "aws_iam_role_policy_attachment" "aws_six_layer_lambda_logs" {
  count      = local.aws_six_layer_event_enabled || local.aws_six_layer_l5_enabled ? 1 : 0
  role       = aws_iam_role.aws_six_layer_lambda[0].name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_role_policy" "aws_six_layer_lambda_data" {
  count = local.aws_six_layer_event_enabled || local.aws_six_layer_l5_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-runtime-data"
  role  = aws_iam_role.aws_six_layer_lambda[0].id

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = concat(
      local.aws_six_layer_embedded_event_enabled || local.aws_six_layer_remote_telemetry_inbound || local.aws_six_layer_remote_control_inbound ? [{
        Effect = "Allow"
        ...
        Resource = compact([
          ...
          try(aws_sqs_queue.aws_six_layer_embedded_failure[0].arn, ""),
          try(aws_sqs_queue.aws_six_layer_remote_failure[0].arn, ""),
          try(aws_sqs_queue.aws_six_layer_remote_control[0].arn, ""),
        ])
      }] : [],
      local.aws_six_layer_remote_telemetry_enabled ? [{
        Effect   = "Allow"
      ...
      local.aws_six_layer_remote_control_enabled ? [{
        Effect   = "Allow"
        Action   = ["sns:Publish"]
      ...
      local.aws_six_layer_l1_enabled ? [{
        Effect = "Allow"
      ...
      local.aws_six_layer_l2_enabled ? [{
        ...
        Resource = [
          aws_lambda_function.aws_six_layer_extension_action[0].arn,
          one(values(aws_lambda_function.aws_six_layer_processor_extension)).arn,
        ]
      }] : [],
      local.aws_six_layer_l2_enabled ? [{
        Effect   = "Allow"
...
resource "aws_lambda_function" "aws_aws_lambda_event_adapter" {
  count         = local.aws_six_layer_event_enabled ? 1 : 0
  function_name = "${local.aws_six_layer_name}-six-event-adapter"
  role          = aws_iam_role.aws_six_layer_lambda[0].arn
  handler       = "handler.event_adapter"
  runtime       = local.python_runtime_aws
  timeout       = 30
  memory_size   = 512

  filename         = local.aws_six_layer_runtime_package
  source_code_hash = filebase64sha256(local.aws_six_layer_runtime_package)
  ...
  environment {
    variables = {
      ...
      LOCAL_PROCESSING             = tostring(local.aws_six_layer_l2_enabled && !local.six_layer_eventing_enabled)
      ...
      STORAGE_TASK_COUNT           = tostring(local.aws_six_layer_storage_task_count)
...
resource "aws_iot_thing" "aws_aws_iot_core" {
  count = local.aws_six_layer_l1_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-poc-gateway"
  attributes = {
    architecture_profile = "${var.architecture_profile_id}-${var.architecture_profile_version}"
  }
}
...
resource "aws_iot_topic_rule" "aws_aws_iot_core" {
  count       = local.aws_six_layer_l1_enabled ? 1 : 0
  name        = replace("${local.aws_six_layer_name}_v2_telemetry", "-", "_")
  description = "Six-layer telemetry ingress"
  enabled     = true
...
resource "aws_lambda_permission" "aws_six_layer_iot_event_adapter" {
  count         = local.aws_six_layer_l1_enabled ? 1 : 0
  statement_id  = "AllowSixLayerIoT"
  action        = "lambda:InvokeFunction"
...
resource "awscc_iot_command" "aws_aws_iot_commands" {
  count        = local.aws_six_layer_l1_enabled ? 1 : 0
  command_id   = "${local.aws_six_layer_name}-device-command"
...
resource "aws_lambda_function" "aws_aws_lambda" {
  count         = local.aws_six_layer_l2_enabled ? 1 : 0
  function_name = "${local.aws_six_layer_name}-six-processor"
  role          = aws_iam_role.aws_six_layer_lambda[0].arn
  ...
  environment {
    variables = {
      ...
      PROCESSOR_EXTENSION_FUNCTION_NAME = try(one(values(aws_lambda_function.aws_six_layer_processor_extension)).function_name, "")
...
resource "aws_iam_role" "aws_six_layer_extension_action" {
  count = local.aws_six_layer_l2_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-poc-action-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect    = "Allow"
...
resource "aws_iam_role" "aws_six_layer_processor_extension" {
  count = local.aws_six_layer_l2_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-processor-extension-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect    = "Allow"
...
resource "aws_iam_role_policy_attachment" "aws_six_layer_processor_extension_logs" {
  count      = local.aws_six_layer_l2_enabled ? 1 : 0
  role       = aws_iam_role.aws_six_layer_processor_extension[0].name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_lambda_function" "aws_six_layer_processor_extension" {
  for_each      = local.aws_six_layer_processor_extensions
  function_name = "${local.aws_six_layer_name}-six-processor-extension"
  role          = aws_iam_role.aws_six_layer_processor_extension[0].arn
  ...
  lifecycle {
    precondition {
      ...
      error_message = "AWS Six-layer requires the reviewed processor.telemetry@1 AWS adapter."
    }
  }
...
resource "aws_iam_role_policy_attachment" "aws_six_layer_extension_action_logs" {
  count      = local.aws_six_layer_l2_enabled ? 1 : 0
  role       = aws_iam_role.aws_six_layer_extension_action[0].name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_lambda_function" "aws_six_layer_extension_action" {
  count         = local.aws_six_layer_l2_enabled ? 1 : 0
  function_name = "${local.aws_six_layer_name}-six-poc-action"
  role          = aws_iam_role.aws_six_layer_extension_action[0].arn
...
resource "aws_lambda_event_source_mapping" "aws_six_layer_embedded_events" {
  count                              = local.aws_six_layer_l2_enabled && local.aws_six_layer_embedded_event_enabled ? 1 : 0
...
resource "aws_lambda_function" "aws_six_layer_domain_consumer" {
  count         = local.aws_six_layer_domain_consumer_enabled ? 1 : 0
  function_name = "${local.aws_six_layer_name}-six-domain-consumer"
  role          = aws_iam_role.aws_six_layer_lambda[0].arn
  handler       = "handler.domain_consumer"
  runtime       = local.python_runtime_aws
  timeout       = 30
...
resource "aws_lambda_event_source_mapping" "aws_six_layer_remote_telemetry" {
  count                              = local.aws_six_layer_remote_telemetry_inbound ? 1 : 0
  event_source_arn                   = aws_kinesis_stream.aws_aws_kinesis_only_for_reviewed_remote_telemetry_edge["inbound"].arn
  function_name                      = aws_lambda_function.aws_six_layer_domain_consumer[0].arn
  ...
  destination_config {
    on_failure {
      destination_arn = aws_sqs_queue.aws_six_layer_remote_failure[0].arn
    }
  }
  enabled = true
}
...
resource "aws_sns_topic_subscription" "aws_aws_sns_fifo_only_for_reviewed_remote_control_edge" {
  ...
  protocol             = "sqs"
  endpoint             = aws_sqs_queue.aws_six_layer_remote_control[0].arn
  raw_message_delivery = true
}

resource "aws_lambda_event_source_mapping" "aws_six_layer_remote_control" {
  count                              = local.aws_six_layer_remote_control_inbound ? 1 : 0
  event_source_arn                   = aws_sqs_queue.aws_six_layer_remote_control[0].arn
  function_name                      = aws_lambda_function.aws_six_layer_domain_consumer[0].arn
  batch_size                         = 1
  maximum_batching_window_in_seconds = 0
  function_response_types            = ["ReportBatchItemFailures"]
...
resource "aws_iam_role" "aws_six_layer_step_functions" {
  count = local.aws_six_layer_l2_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-workflow-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect    = "Allow"
...
resource "aws_iam_role_policy" "aws_six_layer_step_functions" {
  count = local.aws_six_layer_l2_enabled ? 1 : 0
  name  = "invoke-six-processor"
  role  = aws_iam_role.aws_six_layer_step_functions[0].id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
...
resource "aws_sfn_state_machine" "aws_aws_step_functions_standard" {
  count    = local.aws_six_layer_l2_enabled ? 1 : 0
  name     = "${local.aws_six_layer_name}-six-event-workflow"
  role_arn = aws_iam_role.aws_six_layer_step_functions[0].arn
  type     = "STANDARD"
  definition = jsonencode({
    Comment = "Fixed four-action Six-layer notification workflow"
    ...
    States = {
      ...
      DeliverNotification = {
        ...
        Parameters = {
          FunctionName = aws_lambda_function.aws_six_layer_extension_action[0].arn
          Payload = {
...
resource "aws_dynamodb_table" "aws_aws_dynamodb_on_demand_raw" {
  count        = local.aws_six_layer_hot_enabled ? 1 : 0
  name         = "${local.aws_six_layer_name}-six-raw"
...
resource "aws_dynamodb_table" "aws_aws_dynamodb_on_demand_hourly_rollup" {
  count        = local.aws_six_layer_hot_enabled ? 1 : 0
  name         = "${local.aws_six_layer_name}-six-hourly-rollup"
  billing_mode = "PAY_PER_REQUEST"
...
resource "aws_s3_bucket" "aws_aws_s3_standard_ia" {
  count         = local.aws_six_layer_object_store_enabled ? 1 : 0
  bucket        = "${local.aws_six_layer_name}-six-history-${local.deployment_suffix}"
  force_destroy = true
  tags          = local.aws_six_layer_tags
}

resource "aws_s3_bucket_public_access_block" "aws_six_layer_history" {
  count                   = local.aws_six_layer_object_store_enabled ? 1 : 0
...
resource "aws_s3_bucket_server_side_encryption_configuration" "aws_six_layer_history" {
  count  = local.aws_six_layer_object_store_enabled ? 1 : 0
  bucket = aws_s3_bucket.aws_aws_s3_standard_ia[0].id
  rule {
...
resource "aws_s3_bucket_lifecycle_configuration" "aws_aws_s3_standard_ia" {
  count  = local.aws_six_layer_cool_enabled ? 1 : 0
  bucket = aws_s3_bucket.aws_aws_s3_standard_ia[0].id

  rule {
    id     = "six-layer-cool-history"
    status = "Enabled"
    filter { prefix = "history/" }

    dynamic "transition" {
      for_each = local.aws_six_layer_archive_enabled ? [1] : []
      content {
    ...
    expiration {
      days = local.aws_six_layer_archive_enabled ? (
...
resource "aws_s3_bucket_lifecycle_configuration" "aws_aws_s3_glacier_deep_archive" {
  count  = local.aws_six_layer_archive_enabled && !local.aws_six_layer_cool_enabled ? 1 : 0
  bucket = aws_s3_bucket.aws_aws_s3_standard_ia[0].id

  rule {
    id     = "six-layer-remote-archive"
    status = "Enabled"
    filter { prefix = "history/" }
    expiration {
...
resource "aws_ecr_repository" "aws_aws_ecr_if_container_selected" {
  count                = local.aws_six_layer_image_foundation_enabled ? 1 : 0
  name                 = "${local.aws_six_layer_name}-six-images"
...
resource "aws_s3_bucket" "aws_aws_ecr_if_container_selected" {
  count         = local.aws_six_layer_image_foundation_enabled ? 1 : 0
  bucket        = "${local.aws_six_layer_name}-six-build-${local.deployment_suffix}"
  force_destroy = true
  tags          = local.aws_six_layer_tags
}
...
resource "aws_iam_role" "aws_aws_ecr_if_container_selected" {
  count = local.aws_six_layer_image_foundation_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-build-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version   = "2012-10-17"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current[0].account_id}:log-group:/aws/codebuild/${local.aws_six_layer_name}-six-images*",
        ]
      },
    ]
  })
...
resource "aws_codebuild_project" "aws_aws_ecr_if_container_selected" {
  count         = local.aws_six_layer_image_foundation_enabled ? 1 : 0
  name          = "${local.aws_six_layer_name}-six-images"
  description   = "Bounded Six-layer container publication"
...
resource "terraform_data" "aws_six_layer_storage_mover_guard" {
  count = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  input = {
    image      = var.aws_six_layer_storage_mover_image
    task_count = local.aws_six_layer_storage_task_count
  }
  lifecycle {
    precondition {
      condition     = contains([1, 3], local.aws_six_layer_storage_task_count)
      error_message = "AWS Six-layer storage movement requires the exact reviewed 1/1/3 task_count capacity dimension."
    }
    precondition {
      condition = (
        var.aws_six_layer_storage_mover_image != "" &&
        startswith(var.aws_six_layer_storage_mover_image, "${aws_ecr_repository.aws_aws_ecr_if_container_selected[0].repository_url}@sha256:")
      )
      error_message = "AWS Six-layer storage movement requires a digest-pinned image from the deployment ECR repository."
    }
  }
}

resource "terraform_data" "aws_six_layer_bridge_image_guard" {
  count = length(local.aws_six_layer_outbound_event_routes) > 0 ? 1 : 0
  input = {
    image = var.aws_six_layer_bridge_image
  }
  lifecycle {
    precondition {
      condition = (
        var.aws_six_layer_bridge_image != "" &&
        startswith(var.aws_six_layer_bridge_image, "${aws_ecr_repository.aws_aws_ecr_if_container_selected[0].repository_url}@sha256:")
      )
      error_message = "AWS Six-layer outbound event routes require a digest-pinned bridge image from the deployment ECR repository."
    }
  }
}
...
resource "aws_ecs_cluster" "aws_aws_ecs_fargate_storage_mover" {
  count = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-storage-mover"
  setting {
...
resource "aws_iam_role" "aws_six_layer_ecs_execution" {
  count = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-ecs-exec-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version   = "2012-10-17"
...
resource "aws_iam_role_policy_attachment" "aws_six_layer_ecs_execution" {
  count      = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  role       = aws_iam_role.aws_six_layer_ecs_execution[0].name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy"
}

resource "aws_iam_role" "aws_six_layer_storage_mover" {
  count = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-mover-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [{ Effect = "Allow", Action = "sts:AssumeRole", Principal = { Service = "ecs-tasks.amazonaws.com" } }]
  })
...
resource "aws_iam_role_policy" "aws_six_layer_storage_mover" {
  count = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  name  = "bounded-storage-window"
  role  = aws_iam_role.aws_six_layer_storage_mover[0].id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = concat(
      local.aws_six_layer_hot_enabled ? [{
      ...
      local.aws_six_layer_object_store_enabled ? [{
        Effect = "Allow"
...
resource "aws_vpc" "aws_six_layer_storage_mover" {
  count                = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  cidr_block           = "10.42.0.0/24"
  enable_dns_support   = true
  enable_dns_hostnames = true
  tags                 = merge(local.aws_six_layer_tags, { Name = "${local.aws_six_layer_name}-six-mover" })
}

resource "aws_internet_gateway" "aws_six_layer_storage_mover" {
  count  = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  vpc_id = aws_vpc.aws_six_layer_storage_mover[0].id
  tags   = local.aws_six_layer_tags
}

resource "aws_subnet" "aws_six_layer_storage_mover" {
  count                   = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  vpc_id                  = aws_vpc.aws_six_layer_storage_mover[0].id
...
resource "aws_route_table" "aws_six_layer_storage_mover" {
  count  = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  vpc_id = aws_vpc.aws_six_layer_storage_mover[0].id
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.aws_six_layer_storage_mover[0].id
  }
  tags = local.aws_six_layer_tags
}

resource "aws_route_table_association" "aws_six_layer_storage_mover" {
  count          = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  subnet_id      = aws_subnet.aws_six_layer_storage_mover[0].id
  route_table_id = aws_route_table.aws_six_layer_storage_mover[0].id
}

resource "aws_security_group" "aws_six_layer_storage_mover" {
  count       = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  name_prefix = "${local.aws_six_layer_name}-six-mover-"
...
resource "aws_ecs_task_definition" "aws_aws_ecs_fargate_storage_mover" {
  count                    = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  family                   = "${local.aws_six_layer_name}-six-storage-mover"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = "1024"
  memory                   = "2048"
  execution_role_arn       = aws_iam_role.aws_six_layer_ecs_execution[0].arn
  task_role_arn            = aws_iam_role.aws_six_layer_storage_mover[0].arn
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "STORAGE_TASK_COUNT", value = tostring(local.aws_six_layer_storage_task_count) },
...
resource "aws_iam_role" "aws_six_layer_scheduler" {
  count = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-scheduler-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version   = "2012-10-17"
...
resource "aws_iam_role_policy" "aws_six_layer_scheduler" {
  count = local.aws_six_layer_storage_mover_enabled ? 1 : 0
  name  = "run-finite-storage-mover"
  role  = aws_iam_role.aws_six_layer_scheduler[0].id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      { Effect = "Allow", Action = "ecs:RunTask", Resource = aws_ecs_task_definition.aws_aws_ecs_fargate_storage_mover[0].arn },
      { Effect = "Allow", Action = "iam:PassRole", Resource = [aws_iam_role.aws_six_layer_ecs_execution[0].arn, aws_iam_role.aws_six_layer_storage_mover[0].arn] },
    ]
  })
}

resource "aws_scheduler_schedule" "aws_aws_eventbridge_scheduler" {
  for_each            = local.aws_six_layer_storage_schedule_tasks
  name                = "${local.aws_six_layer_name}-six-${each.key}"
  ...
  target {
    ...
    role_arn = aws_iam_role.aws_six_layer_scheduler[0].arn
    ecs_parameters {
      ...
      network_configuration {
        ...
        subnets          = [aws_subnet.aws_six_layer_storage_mover[0].id]
        security_groups  = [aws_security_group.aws_six_layer_storage_mover[0].id]
      }
    }
    input = jsonencode({
      containerOverrides = [{
...
resource "aws_s3_bucket" "aws_six_layer_twinmaker" {
  count         = local.aws_six_layer_l4_enabled ? 1 : 0
  bucket        = "${local.aws_six_layer_name}-six-twinmaker-${local.deployment_suffix}"
  force_destroy = true
  tags          = local.aws_six_layer_tags
}

resource "aws_s3_bucket_public_access_block" "aws_six_layer_twinmaker" {
  count                   = local.aws_six_layer_l4_enabled ? 1 : 0
  bucket                  = aws_s3_bucket.aws_six_layer_twinmaker[0].id
  block_public_acls       = true
  block_public_policy     = true
  ignore_public_acls      = true
  restrict_public_buckets = true
...
resource "aws_iam_role" "aws_six_layer_twinmaker" {
  count = local.aws_six_layer_l4_enabled ? 1 : 0
  name  = "${local.aws_six_layer_name}-six-twinmaker-${local.deployment_suffix}"
  assume_role_policy = jsonencode({
    Version   = "2012-10-17"
...
resource "aws_iam_role_policy" "aws_six_layer_twinmaker" {
  count = local.aws_six_layer_l4_enabled ? 1 : 0
  name  = "workspace-bucket"
  role  = aws_iam_role.aws_six_layer_twinmaker[0].id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect   = "Allow"
      Action   = ["s3:GetBucketLocation", "s3:ListBucket", "s3:GetObject", "s3:PutObject", "s3:DeleteObject"]
      Resource = [aws_s3_bucket.aws_six_layer_twinmaker[0].arn, "${aws_s3_bucket.aws_six_layer_twinmaker[0].arn}/*"]
    }]
  })
}
...
resource "awscc_iottwinmaker_workspace" "aws_aws_iot_twinmaker_standard" {
  count        = local.aws_six_layer_l4_enabled ? 1 : 0
  workspace_id = "${local.aws_six_layer_name}-${local.deployment_suffix}"
  description  = "Six-layer PoC twin workspace"
  role         = aws_iam_role.aws_six_layer_twinmaker[0].arn
  s3_location  = aws_s3_bucket.aws_six_layer_twinmaker[0].arn
  tags         = local.aws_six_layer_tags
  depends_on   = [aws_iam_role_policy.aws_six_layer_twinmaker]
}
...
resource "aws_lambda_function" "aws_aws_lambda_raw_history_reader" {
  ...
  function_name                  = "${local.aws_six_layer_name}-six-layer-raw-history-reader"
  role                           = aws_iam_role.aws_six_layer_lambda[0].arn
...
data "aws_ssoadmin_instances" "aws_six_layer_layer_access" {
  provider = aws.sso
  count    = local.aws_six_layer_layer_access_enabled ? 1 : 0
}

locals {
  aws_six_layer_sso_instance_arn  = try(tolist(data.aws_ssoadmin_instances.aws_six_layer_layer_access[0].arns)[0], "")
  aws_six_layer_identity_store_id = try(tolist(data.aws_ssoadmin_instances.aws_six_layer_layer_access[0].identity_store_ids)[0], "")
}

data "aws_identitystore_users" "aws_six_layer_layer_access" {
  provider          = aws.sso
  count             = local.aws_six_layer_layer_access_enabled ? 1 : 0
  identity_store_id = local.aws_six_layer_identity_store_id
}

locals {
  aws_six_layer_matching_users = local.aws_six_layer_layer_access_enabled ? [
    for user in coalesce(try(data.aws_identitystore_users.aws_six_layer_layer_access[0].users, []), []) :
    user if user.user_name == var.platform_user_email
  ] : []
  aws_six_layer_create_layer_user = (
    local.aws_six_layer_layer_access_enabled &&
    length(local.aws_six_layer_matching_users) == 0 &&
    var.aws_layer_access_principal_intent == "invite_builtin"
  )
}

resource "terraform_data" "aws_six_layer_layer_access_principal_admission" {
  count = local.aws_six_layer_layer_access_enabled ? 1 : 0

  lifecycle {
    precondition {
      condition = (
        length(local.aws_six_layer_matching_users) == 1 ||
        var.aws_layer_access_principal_intent == "invite_builtin"
      )
...
resource "aws_identitystore_user" "aws_six_layer_layer_access" {
  provider          = aws.sso
  count             = local.aws_six_layer_create_layer_user ? 1 : 0
  identity_store_id = local.aws_six_layer_identity_store_id
...
locals {
  aws_six_layer_layer_user_id = local.aws_six_layer_layer_access_enabled ? (
    length(local.aws_six_layer_matching_users) > 0 ?
    local.aws_six_layer_matching_users[0].user_id :
    try(aws_identitystore_user.aws_six_layer_layer_access[0].user_id, "")
  ) : ""
}
...
resource "aws_ssoadmin_permission_set_inline_policy" "aws_six_layer_layer_access" {
  provider           = aws.sso
  count              = local.aws_six_layer_layer_access_enabled ? 1 : 0
  instance_arn       = local.aws_six_layer_sso_instance_arn
...
resource "aws_ssoadmin_account_assignment" "aws_aws_iam_identity_center_layer_access" {
  ...
  instance_arn       = local.aws_six_layer_sso_instance_arn
  permission_set_arn = aws_ssoadmin_permission_set.aws_aws_iam_identity_center_layer_access[0].arn
  principal_id       = local.aws_six_layer_layer_user_id
  principal_type     = "USER"
...
output "aws_component_ingestion_output" {
  value = local.aws_six_layer_l1_enabled ? {
...
output "aws_component_processing_output" {
  value = local.aws_six_layer_l2_enabled ? {
...
output "aws_component_hot_storage_output" {
  value = local.aws_six_layer_hot_enabled ? {
...
output "aws_component_cool_storage_output" {
  value = local.aws_six_layer_cool_enabled ? { bucket = aws_s3_bucket.aws_aws_s3_standard_ia[0].bucket } : null
}

output "aws_component_archive_storage_output" {
  value = local.aws_six_layer_archive_enabled ? { bucket = aws_s3_bucket.aws_aws_s3_standard_ia[0].bucket } : null
}

output "aws_component_twin_state_output" {
  value = local.aws_six_layer_l4_enabled ? {
