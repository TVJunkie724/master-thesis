...
class DiagnosticSettingsHelper:
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                resp = requests.request(
                    method, url, headers=self._get_headers(), timeout=self.TIMEOUT
                )
                last_response = resp
    ...
    def delete(self, resource_id: str, setting_name: str) -> bool:
        ...
        if resp.status_code == 404:
            logger.debug(
                f"Diagnostic setting '{setting_name}' not found (already deleted?)"
            )
            return False
    ...
    def cleanup_orphaned_by_prefix(self, prefix: str, dry_run: bool = False) -> dict:
        ...
        logger.info(
            f"[Diagnostic Settings] Scanning subscription for orphans matching '{prefix}'..."
        )

        # Check all ADT instances
        try:
            adt_client = AzureDigitalTwinsManagementClient(
                ...
                if resource_name_owned_by_prefix(
                    ...
                    for setting in self.list(blob_id):
                        ...
                        if resource_name_owned_by_prefix(
                            ...
                            results["found"] += 1
                            logger.info(
                                f"  Storage '{account.name}/blobServices/default': {setting_name}"
                            )
                            if dry_run:
                                logger.info("    [DRY RUN] Would delete")
                            else:
        ...
        logger.info(
            f"[Diagnostic Settings] Scan complete: {results['resources_checked']} resources checked, "
