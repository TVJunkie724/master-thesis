...
def _query_dynamodb(query_event: dict) -> dict:
    """Execute the DynamoDB query and return TwinMaker-compatible response."""
    ...
    for property_name in query_event["selectedProperties"]:
        property_type = f"{query_event['properties'][property_name]['definition']['dataType']['type'].capitalize()}Value"
...
def lambda_handler(event, context):
    ...
    try:
        ...
        result = _query_dynamodb(event)
        return result

    except Exception as e:
        print(f"Hot Reader Error: {e}")
        # For HTTP requests, return proper error response
        if _is_http_request(event):
            return {
                "statusCode": 500,
