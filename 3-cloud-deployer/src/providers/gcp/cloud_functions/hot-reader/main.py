...
try:
    from _shared.inter_cloud import build_auth_error_response, redact_diagnostic, validate_token
...
def main(request):
    ...
    try:
        ...
        print(f"Hot Reader Error: {diagnostic}")
        traceback.print_exc()
        return (json.dumps({"error": diagnostic}), 500, {"Content-Type": "application/json"})
