...
def gcp_six_layer_container_deployment(tfvars: Mapping[str, Any]) -> bool:
    profile = (
...
def _gcp_six_layer_platform_deployment(tfvars: Mapping[str, Any]) -> bool:
    phase8_profile = (
        tfvars.get("architecture_profile_id"),
        str(tfvars.get("architecture_profile_version")),
...
def placeholder_image_tfvars(tfvars: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not gcp_six_layer_container_deployment(tfvars):
        return {}
    prefix = (
        f"{tfvars['gcp_region']}-docker.pkg.dev/{tfvars['gcp_project_id']}/"
        f"{_normalized_name(str(tfvars['digital_twin_name']))}-six-layer/"
    )
    return {
...
def image_requests(
...
) -> tuple[GcpSixLayerImageRequest, ...]:
    domain_context = (
        "six-layer-domain.tar.gz"
        if tfvars.get("architecture_profile_id") == "six-layer-eventing"
        and str(tfvars.get("architecture_profile_version")) == "1"
        else "six-layer-domain.tar.gz"
    )
    platform = Path(project_path) / ".build" / "gcp" / domain_context
    requests: list[GcpSixLayerImageRequest] = []
    if _gcp_six_layer_platform_deployment(tfvars):
        requests.append(GcpSixLayerImageRequest("platform", platform))
    ...
    if tfvars.get("layer_3_hot_provider") == "google" or (
...
class GcpSixLayerImagePublisher:
    ...
    def from_tfvars_and_outputs(
        ...
        *,
        ...
        required = {
            "source_bucket": outputs.get("gcp_six_layer_build_source_bucket"),
            "registry_prefix": outputs.get("gcp_six_layer_registry_prefix"),
            "build_service_account": outputs.get("gcp_six_layer_build_service_account"),
        }
    ...
    def _cached(
        self, requests: tuple[GcpSixLayerImageRequest, ...]
    ) -> dict[str, str] | None:
    ...
    def publish(self, requests: tuple[GcpSixLayerImageRequest, ...]) -> dict[str, str]:
    ...
    def _publish_one(self, request: GcpSixLayerImageRequest) -> str:
        ...
        if not isinstance(digest, str) or not _DIGEST_REF.fullmatch(result):
            raise RuntimeError(
                f"Cloud Build returned no image digest for {request.name}"
            )
        return result
...
def image_tfvars(
    ...
    if _gcp_six_layer_platform_deployment(tfvars):
        result["gcp_six_layer_platform_image"] = images.get("platform", "")
    if _gcp_eventing_deployment(tfvars):
        result["gcp_event_runtime_image"] = images.get("event-runtime", "")
    if tfvars.get("layer_2_provider") == "google":
        result["gcp_six_layer_processor_extension_image"] = images.get(
            "processor-extension", ""
        )
    if tfvars.get("layer_3_hot_provider") == "google" or (
        tfvars.get("layer_3_cold_provider") == "google"
