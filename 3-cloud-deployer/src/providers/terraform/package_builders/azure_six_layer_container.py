"""Deterministic ACR Task context for the Azure Six-layer mover."""
...
PACKAGE_ID = "azure_six-layer-domain-storage-mover"
...
def build_azure_six_layer_storage_mover_context(project_path: Path) -> Dict[str, Path]:
    source = (
        PROVIDERS_ROOT
        / "azure"
        / "azure_functions"
    ...
    if not source.is_dir() or source.is_symlink():
        raise ValueError("Unavailable Azure Six-layer storage-mover context")
    output = (
        Path(project_path)
        / ".build"
        / "azure"
...
__all__ = ["PACKAGE_ID", "build_azure_six_layer_storage_mover_context"]
