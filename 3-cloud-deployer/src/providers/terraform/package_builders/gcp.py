    ...
    *,
    ...
    has_gcp = (
        ...
        if selected_function_names is not None
            ...
            providers_config.get(layer) in {"gcp", "google"} for layer in gcp_layers
        )
    )
    ...
    functions_to_build = (
        ...
        if selected_function_names is not None
        else get_functions_for_provider_build(
            "gcp",
            providers_config,
            optimization_flags,
        )
        ...
        for file_path in sorted(func_dir.rglob("*")):
            if file_path.is_file() and _should_include_file(file_path):
                ...
                if file_path.name == "main.py" and digital_twin_name and device_id:
                    ...
                    content = _rewrite_gcp_function_names(
...
def _create_gcp_processor_zip(
    base_dir: Path, user_dir: Path, shared_dir: Path, output_path: Path
) -> None:
    """
