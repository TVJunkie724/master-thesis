"""Deterministic container contexts for standalone GCP Six-layer."""
...
def build_gcp_six_layer_container_contexts(
    project_path: Path,
    ...
    unknown = selected - GCP_SIX_LAYER_CONTEXTS
    if unknown:
        raise ValueError(f"Unknown GCP Six-layer context: {sorted(unknown)}")
    if not selected:
        return {}
    ...
    for name in sorted(selected):
        ...
        if not source.is_dir() or source.is_symlink():
            raise ValueError(f"Unavailable GCP Six-layer context: {name}")
...
def build_gcp_six_layer_extension_container_context(
    project_path: Path,
    ...
    files: dict[str, bytes] = {
        "Dockerfile": GCP_SIX_LAYER_EXTENSION_DOCKERFILE.encode()
    }
