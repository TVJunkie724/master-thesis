"""Automatic content-addressed AWS image publication for Six-layer."""
...
_EVIDENCE_VERSION = "aws-six-layer-published-images.v1"


@dataclass(frozen=True, slots=True)
class AwsSixLayerImageRequest:
    name: str
    context: Path


def _six_layer(tfvars: Mapping[str, Any]) -> bool:
    """Return whether the canonical Six-layer profile is selected."""
...
def aws_six_layer_storage_mover_deployment(tfvars: Mapping[str, Any]) -> bool:
    if not _six_layer(tfvars):
        return False
...
def aws_six_layer_bridge_deployment(tfvars: Mapping[str, Any]) -> bool:
    if not _six_layer(tfvars):
        return False
...
def aws_six_layer_container_deployment(tfvars: Mapping[str, Any]) -> bool:
    return aws_six_layer_storage_mover_deployment(
        tfvars
    ) or aws_six_layer_bridge_deployment(tfvars)


def image_requests(
    project_path: Path, tfvars: Mapping[str, Any]
) -> tuple[AwsSixLayerImageRequest, ...]:
    requests: list[AwsSixLayerImageRequest] = []
    build_root = Path(project_path) / ".build" / "aws"
    bridge_context = "six-layer-domain-bridge.zip"
    selections = (
        (
            aws_six_layer_storage_mover_deployment(tfvars),
            "storage-mover",
            build_root / "six-layer-domain-storage-mover.zip",
        ),
        (
            aws_six_layer_bridge_deployment(tfvars),
            "bridge",
    ...
    for selected, name, context in selections:
        ...
        requests.append(AwsSixLayerImageRequest(name, context))
...
class AwsSixLayerImagePublisher:
    ...
    def from_tfvars_and_outputs(
        ...
        *,
        ...
        required = {
            "source_bucket": outputs.get("aws_six_layer_build_source_bucket"),
            "repository_url": outputs.get("aws_six_layer_ecr_repository_url"),
            "repository_name": outputs.get("aws_six_layer_ecr_repository_name"),
            "codebuild_project": outputs.get("aws_six_layer_codebuild_project"),
        }
    ...
    def _cached(
        self, requests: tuple[AwsSixLayerImageRequest, ...]
    ) -> dict[str, str] | None:
        ...
        cached = dict(images)
        return (
            cached if all(self._digest_exists(ref) for ref in cached.values()) else None
        )
    ...
    def publish(self, requests: tuple[AwsSixLayerImageRequest, ...]) -> dict[str, str]:
    ...
    def _publish_one(self, request: AwsSixLayerImageRequest) -> str:
...
def image_tfvars(
    ...
    expected = {
        ...
        for selected, name, variable in (
            (
                aws_six_layer_storage_mover_deployment(tfvars),
                "storage-mover",
                "aws_six_layer_storage_mover_image",
            ),
            (
                aws_six_layer_bridge_deployment(tfvars),
                "bridge",
                "aws_six_layer_bridge_image",
            ),
        )
        if selected
    }
    ...
    if set(images) != set(expected) or any(not images.get(name) for name in expected):
        raise RuntimeError("AWS Six-layer image publication is incomplete")
    return {variable: images[name] for name, variable in expected.items()}


__all__ = [
    "AwsSixLayerImagePublisher",
    "AwsSixLayerImageRequest",
    "aws_six_layer_bridge_deployment",
    "aws_six_layer_container_deployment",
    "aws_six_layer_storage_mover_deployment",
    "image_requests",
    "image_tfvars",
]
