"""Content-addressed Azure ACR Task publication for Six-layer."""
...
_DIGEST_REF = re.compile(r"^[a-z0-9]+\.azurecr\.io/[a-z0-9_./-]+@sha256:[0-9a-f]{64}$")
_EVIDENCE_VERSION = "azure-six-layer-published-images.v1"


@dataclass(frozen=True, slots=True)
class AzureSixLayerImageRequest:
    name: str
    context: Path


def azure_six_layer_container_deployment(tfvars: Mapping[str, Any]) -> bool:
    profile = (
...
) -> tuple[AzureSixLayerImageRequest, ...]:
    if not azure_six_layer_container_deployment(tfvars):
        return ()
    context = (
        Path(project_path)
        / ".build"
        / "azure"
        / "six-layer-domain-storage-mover.tar.gz"
    )
    if not context.is_file() or context.is_symlink():
        raise ValueError("Azure storage-mover image context is unavailable")
    return (AzureSixLayerImageRequest("storage-mover", context),)
...
class AzureSixLayerImagePublisher:
    ...
    def from_tfvars_and_outputs(
        ...
        *,
        ...
        required = {
            "resource_group": outputs.get("azure_six_layer_resource_group_name"),
            "location": outputs.get("azure_six_layer_location"),
            "registry_name": outputs.get("azure_six_layer_acr_name"),
            "login_server": outputs.get("azure_six_layer_acr_login_server"),
        }
    ...
    @staticmethod
    def _run_name(request: AzureSixLayerImageRequest) -> str:
        return f"t2mc-{request.name}-{_digest(request.context)[:16]}"

    def _get_run(self, request: AzureSixLayerImageRequest) -> dict[str, Any] | None:
    ...
    def _image_from_run(
        self, request: AzureSixLayerImageRequest, run: Mapping[str, Any]
    ) -> str | None:
    ...
    def _wait_for_run(self, request: AzureSixLayerImageRequest) -> str:
        for _ in range(600):
    ...
    def _delete_run(self, request: AzureSixLayerImageRequest) -> None:
        self._arm(
            "DELETE",
            self._task_run_url(self._run_name(request)),
    ...
    def _cached(
        self, requests_: tuple[AzureSixLayerImageRequest, ...]
    ) -> dict[str, str] | None:
    ...
    def _publish_one(self, request: AzureSixLayerImageRequest) -> str:
        ...
        if not all(
            isinstance(value, str) and value for value in (upload_url, relative_path)
        ):
...
def image_tfvars(
    ...
    return {"azure_six_layer_storage_mover_image": image}


__all__ = [
    "AzureSixLayerImagePublisher",
    "AzureSixLayerImageRequest",
