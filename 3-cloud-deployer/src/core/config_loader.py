...
class ProjectConfigLoader:
    ...
    def load_bundle_from_path(
        ...
        return ProjectConfigBundle(
            ...
            resolved_deployment_graph=(
                ...
                and validated_manifest.manifest_version == "3.0"
                else None
            ),
        )
    ...
    def create_context(
        ...
        *,
        ...
        context = self.storage.context(project_name)
        return self.create_context_from_path(
            project_name,
    ...
    def create_context_from_path(
        ...
        *,
        ...
        """Create a context from an explicitly acquired operation package path."""
        bundle = self.load_bundle_from_path(project_name, project_path)
...
def normalize_provider_mapping(providers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in providers.items():
        normalized[key] = (
            normalize_provider_name(value) if isinstance(value, str) else value
        )
    return normalized
...
def _load_json_file(file_path: Path, required: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigurationError(
            f"Invalid JSON in configuration file: {e}", config_file=str(file_path)
        )
...
def _load_hierarchy_for_provider(project_path: Path, provider: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigurationError(
            f"Invalid JSON in hierarchy file: {e}", config_file=str(hierarchy_file)
        )
...
def load_project_config(project_path: Path) -> ProjectConfig:
    ...
    inter_cloud = _load_json_file(
        project_path / CONFIG_INTER_CLOUD_FILE, required=False
    )
    ...
    return ProjectConfig(
        ...
        if isinstance(iot_devices, list)
        else iot_devices.get("devices", []),
...
def save_inter_cloud_connection(
    project_path: Path, conn_id: str, url: str, token: str
) -> None:
    """
    ...
    inter_cloud["connections"][conn_id] = {"url": url, "token": token}
    ...
    logger.info(
        f"Saved inter-cloud connection '{conn_id}' to {CONFIG_INTER_CLOUD_FILE}"
    )
...
def load_credentials(project_path: Path) -> Dict[str, dict]:
    ...
    aws_creds = _load_json_file(
        project_path / "config_credentials_aws.json", required=False
    )
    if aws_creds:
    ...
    azure_creds = _load_json_file(
        project_path / "config_credentials_azure.json", required=False
    )
    if azure_creds:
    ...
    gcp_creds = _load_json_file(
        project_path / "config_credentials_google.json", required=False
    )
    if gcp_creds:
