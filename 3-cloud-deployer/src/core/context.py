...
class DeploymentContext:
    ...
    Attributes:
        ...
        deployment_manifest: Optional DeploymentManifest metadata for manifest-backed packages
    ...
    # Optional manifest metadata generated by the Management API.
    # Legacy projects remain supported with an empty manifest.
    deployment_manifest: Dict[str, Any] = field(default_factory=dict)

    # Canonical typed deployment contract. It is mandatory for deploy/destroy,
    # while legacy projects may still be loaded for diagnostics.
    validated_deployment_manifest: Optional["ValidatedDeploymentManifest"] = None

    # Deterministic graph compiled from Manifest v4 before package/Terraform work.
    ...
    deployment_access_runtime_evidence: Optional["DeploymentAccessRuntimeEvidence"] = (
        None
    )
    ...
    @property
    def is_manifest_backed(self) -> bool:
        """Return True when the context was created from a manifest-backed package."""
        return bool(self.deployment_manifest)

    @property
    def manifest_resource_name(self) -> Optional[str]:
        """Return twin.resource_name from deployment_manifest.json, if present."""
        twin = self.deployment_manifest.get("twin")
        if not isinstance(twin, dict):
            return None
        resource_name = twin.get("resource_name")
        return resource_name if isinstance(resource_name, str) else None

    def validate_manifest_identity(self) -> None:
        """Ensure manifest-backed contexts cannot drift from their runtime project."""
        if not self.deployment_manifest:
            return
        resource_name = self.manifest_resource_name
        if not resource_name:
            raise ValueError("deployment_manifest.json twin.resource_name is required.")
        if resource_name != self.project_name:
            raise ValueError(
                "deployment_manifest.json twin.resource_name does not match DeploymentContext project_name."
            )
