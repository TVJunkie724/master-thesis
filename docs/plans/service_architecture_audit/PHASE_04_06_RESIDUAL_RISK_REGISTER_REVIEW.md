...
| Gate | Evidence |
|---|---|
| 4.1 Contract gate | OpenAPI snapshots: Management API 44 paths, Optimizer 23 paths; current Deployer schema has 45 paths and 50 unique operations. |
| 4.2 Test gate | Management API `454 passed`; current Optimizer gate `504 passed`; current Deployer safe gate `1175 passed, 1 skipped` with warnings treated as errors. |
| 4.3 Security gate | Management API, Optimizer, and Deployer are all-severity Bandit clean; Deployer runtime hardening is closed by [#106](https://github.com/TVJunkie724/master-thesis/issues/106). |
...
| The PoC intentionally uses externally provisioned non-root admin credentials. | The thesis does not prove least-privilege deployment identities. | Document as future hardening; do not add credential provisioning to the PoC. | [#79](https://github.com/TVJunkie724/master-thesis/issues/79) |
...
| Local credential fixture files still exist for compatibility. | Test fixtures can be confused with real provider access. | Keep fixtures synthetic and document that real admin credentials are entered externally for supervised PoC runs. | [#6](https://github.com/TVJunkie724/master-thesis/issues/6), [#8](https://github.com/TVJunkie724/master-thesis/issues/8), [#9](https://github.com/TVJunkie724/master-thesis/issues/9), [#10](https://github.com/TVJunkie724/master-thesis/issues/10) |
