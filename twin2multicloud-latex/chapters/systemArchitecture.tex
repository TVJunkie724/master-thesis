...
score, or manual component assignment. A repository utility reproduces and
digest-binds an already enumerated candidate. Only a disabled-by-default,
owner-scoped Management endpoint may pass that internal selector during a
supervised evaluation; the public calculation model and normal application
workflow cannot author it.
...
the digest-bound, disabled-by-default supervised evaluation handoff; the normal
UI continues to deploy the cost-selected candidate. Both paths subsequently
use the same selection, graph-derived preflight, Apply, replay, verification,
Destroy, and cleanup lifecycle. Only the Small workload is live-deployable;
Medium and Large retain explicit capacity blockers.
