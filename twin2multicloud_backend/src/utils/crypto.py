...
# Cache derived keys to avoid repeated derivation (key = "user_id:scope_id")
_key_cache: dict[str, Fernet] = {}


def _derive_key(user_id: str, scope_id: str) -> bytes:
    """
    ...
    """
    salt = f"{user_id}:{scope_id}".encode()
    kdf = PBKDF2HMAC(
...
def _get_fernet(user_id: str, scope_id: str) -> Fernet:
    """Get or create Fernet instance for a user-owned encryption scope."""
    cache_key = f"{user_id}:{scope_id}"
    if cache_key not in _key_cache:
        try:
            key = _derive_key(user_id, scope_id)
...
def encrypt_scoped(plaintext: str, user_id: str, scope_id: str) -> str:
    """
    ...
    Args:
        ...
        scope_id: The user-owned resource scope ID
    ...
    if not user_id or not scope_id:
        raise ValueError("user_id and scope_id required for encryption")
    return _get_fernet(user_id, scope_id).encrypt(plaintext.encode()).decode()


def decrypt_scoped(ciphertext: str, user_id: str, scope_id: str) -> str:
    """
    ...
    if not ciphertext:
        return ""
    if not user_id or not scope_id:
        raise ValueError("user_id and scope_id required for decryption")
    try:
        return _get_fernet(user_id, scope_id).decrypt(ciphertext.encode()).decode()
    except InvalidToken:
        raise ValueError("Decryption failed - invalid key or corrupted data")


def encrypt(plaintext: str, user_id: str, twin_id: str) -> str:
    """Encrypt a plaintext string using the legacy user+twin-specific scope."""
    return encrypt_scoped(plaintext, user_id, twin_id)


def decrypt(ciphertext: str, user_id: str, twin_id: str) -> str:
    """Decrypt a ciphertext string using the legacy user+twin-specific scope."""
    return decrypt_scoped(ciphertext, user_id, twin_id)
