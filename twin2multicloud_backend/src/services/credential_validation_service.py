...
class CredentialValidationService:
    ...
    async def validate_stored_with_deployer(
        ...
        if not config:
            raise ValidationError("No configuration found. Save credentials first.")

        credentials = self._stored_deployer_credentials(config, provider, user_id, twin_id)
        if credentials is None:
            return self._not_configured_result(provider)
        ...
        self._set_provider_validated(config, provider, valid)
        self.db.commit()
    ...
    async def validate_inline_with_deployer(self, request: InlineValidationRequest) -> CredentialValidationResult:
        ...
        credentials = self._inline_deployer_credentials(request, provider)
        if credentials is None:
            return CredentialValidationResult(
                provider=provider,
                valid=False,
    ...
    async def validate_inline_dual(self, request: InlineValidationRequest) -> dict[str, Any]:
        ...
        provider = self._normalize_provider(request.provider)
        credentials = self._inline_dual_credentials(request, provider)
        if credentials is None:
            return self._missing_dual_result(provider)
        optimizer_creds, deployer_creds = credentials
    ...
    async def validate_stored_dual(self, twin_id: str, user_id: str, provider: str) -> dict[str, Any]:
        ...
        config = twin.configuration
        if not config:
            raise ValidationError("No configuration found.")

        credentials = self._stored_dual_credentials(config, provider, user_id, twin_id)
        if credentials is None:
            raise ValidationError(f"{provider.upper()} credentials not configured")

        optimizer_creds, deployer_creds = credentials
        result = await self._perform_dual_validation(provider, optimizer_creds, deployer_creds)
        self._set_provider_validated(config, provider, result.get("valid", False))
        self.db.commit()
        return result
