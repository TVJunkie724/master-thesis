...
def deployed_twin_with_optimizer(authenticated_client, db_session):
    ...
    deployer_config = DeployerConfiguration(
        twin_id=twin_id, deployer_digital_twin_name="sim-test-project"
    )
...
@patch(
    ...
    return_value=_prepared("sim-test-project"),
)
@patch(
...
def test_download_simulator_azure_success(
    mock_download, mock_prepare, authenticated_client, db_session
):
...
def test_download_simulator_gcp_success(
    mock_download, mock_prepare, authenticated_client, db_session
):
...
def test_download_simulator_wrong_user(authenticated_client, db_session):
    ...
    twin = db_session.query(DigitalTwin).filter_by(id=twin_id).first()
    other_user = User(email="simulator-other-user@example.test")
    db_session.add(other_user)
    db_session.flush()
    twin.state = TwinState.DEPLOYED
    twin.user_id = other_user.id
    db_session.commit()
