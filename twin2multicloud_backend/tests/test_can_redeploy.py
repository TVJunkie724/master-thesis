...
class TestCanRedeploy:
    ...
    def test_twin_not_found_returns_404(self, authenticated_client):
        ...
        response = client.get(
            "/twins/nonexistent-id-12345/can-redeploy", headers=headers
        )
    ...
    def test_other_users_twin_returns_404(self, authenticated_client):
        ...
        response = client.get(
            "/twins/00000000-0000-0000-0000-000000000000/can-redeploy", headers=headers
        )
    ...
    @pytest.mark.asyncio
    async def test_missing_selected_architecture_cannot_trigger_gcp_cooldown(
        self, auth_client, db, test_twin
    ):
        """Only the selected architecture may determine deployment providers."""
        ...
        async def fake_check_cooldown(self, destroyed_at, uses_gcp_firestore):
            ...
            assert uses_gcp_firestore is False
        ...
        assert response.status_code == 200
        assert response.json() == {"ready": True, "remaining_seconds": 0}
