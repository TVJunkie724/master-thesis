...
def test_update_config_encrypts_aws_credentials(db_session):
    user = _create_user(db_session)
    twin = _create_twin(db_session, user)
    ...
    result = _service(db_session).update_config(twin.id, user.id, update)

    db_session.refresh(twin)
    assert result["aws_configured"] is True
    assert twin.configuration.aws_access_key_id != "AKIAIOSFODNN7EXAMPLE"
    assert decrypt(twin.configuration.aws_access_key_id, user.id, twin.id) == "AKIAIOSFODNN7EXAMPLE"
